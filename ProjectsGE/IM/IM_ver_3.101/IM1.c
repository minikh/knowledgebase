...
//Если имитатор выключен, то передаем концевики в статусы
if (!ST_ON_IMIT)
	{
	ST_DI_ON = DI_ON;
	ST_DI_OF = DI_OF;
	//Если c МУК
	if (CFG_MUK && DI_ON && DI_OF)
		{
		ST_DI_ON = 0;
		ST_DI_OF = 0;
		}
	else if (CFG_MUK && !DI_ON && !DI_OF)
		{
		ST_DI_ON = 1;
		ST_DI_OF = 1;
		}
...
if (!CFG_Disable)
	...
	//if ((!CFG_MUK && ST_DI_ON && ST_DI_OF) || (CFG_MUK && !DI_ON && !DI_OF && IM_2_2))
	if (ST_DI_ON && ST_DI_OF)
		PS_KV_FLT= 1;
	//иначе нет фолта
	else
		PS_KV_FLT= 0;	
	...
	if ((AVT_ON != TMP_AVT_ON) || (AVT_OF != TMP_AVT_OF) || ST_RU_Ban)
		BTN_RU = 0;
	...
	if (((BTN_STOP && BTN_RU) || (AVT_STOP && !BTN_RU)) && (IM_1_2 || IM_c1_2 || IM_2_2))
		{
		TMP_Open = 0;
		TMP_Close = 0;
		}
	...
	//Статус: "Разрешение повтора команд"
	ST_Rep_ON = !ST_DO_ON && TMP_T_Block_ON_EN && (PS_notON || PS_notMove_ON || PS_SPOF);
	ST_Rep_OF = !ST_DO_OF && TMP_T_Block_OF_EN && (PS_notOF || PS_notMove_OF || PS_SPON);
	...
	if ((CFG_T_NotMove != 0) && (IM_2_1 || IM_2_2))
		u_notMove_ON = TMP_Open && /*!PS_FLT &&*/ !ST_DI_ON && (TMP_T_Move_ON >= CFG_T_NotMove);
	else
		u_notMove_ON = 0;
	// формирование сигнала "не страгивание на открытие"
	PS_notMove_ON = ST_DI_OF && u_notMove_ON;

	// формирование условия для сигнала "не страгивание на закрытие"
	if ((CFG_T_NotMove !=0) && (IM_2_1 || IM_2_2))	
		u_notMove_OF = TMP_Close && /*!PS_FLT &&*/ !ST_DI_OF && (TMP_T_Move_OF >= CFG_T_NotMove);
	else
		u_notMove_OF = 0;
	// формирование сигнала "не страгивание на закрытие"
	PS_notMove_OF = ST_DI_ON && u_notMove_OF;
		
		
	// формирование условия для сигнала "не открыт"		
	u_notON	= TMP_Open && (TMP_T_Move_ON >= CFG_T_NotCO);
	// формирование сигнала "не открыт"		
	PS_notON = TMP_Open && !ST_DI_ON && u_notON && !PS_SPOF;

	// формирование условия для сигнала "не закрыт"		
	u_notOF	= TMP_Close && (TMP_T_Move_OF >= CFG_T_NotCO);
	// формирование сигнала "не закрыт"		
	PS_notOF = TMP_Close && !ST_DI_OF && u_notOF && !PS_SPON;	
		
	// Дожим на открытие	
	TMP_T_Press_ON_EN = Timer(&TMP_T_Press_ON, CFG_T_Press, TMP_Open && ST_DI_ON);

	// Дожим на закрытие	
	TMP_T_Press_OF_EN = Timer(&TMP_T_Press_OF, CFG_T_Press, TMP_Close && ST_DI_OF);
	...
	//PS_SPOF	= TMP_Open && TMP_u_SPOF && ((!CFG_MUK && !ST_DI_ON) || (CFG_MUK && DI_ON));
	PS_SPOF	= TMP_Open && TMP_u_SPOF && !ST_DI_ON;
	...
	//PS_SPON	= TMP_Close &&  TMP_u_SPON && ((!CFG_MUK && !ST_DI_OF) || (CFG_MUK && DI_OF));
	PS_SPON	= TMP_Close &&  TMP_u_SPON && !ST_DI_OF;
	...
	if (ST_ON_IMIT) 
		...
		if (DO_OF)
			...
			//Если управление без МУК, то повторяем состояние статусов
			// if (!CFG_MUK)
				// {
				// DI_ON = ST_DI_ON;
				// DI_OF = ST_DI_OF;
...
ST_FLT = 	(PS_notMove_ON || PS_notMove_OF || PS_KCU_ON_FLT || PS_KCU_OF_FLT || 
			PS_notON || PS_notOF || PS_M_Mufft || PS_SPON || PS_SPOF || PS_KV_FLT ||
			PS_Vibr || PS_Loil || PS_U_OF || PS_Poil || 
			PS_TYPE_IM_FLT || PS_T_Block_FLT || PS_T_KCU_FLT || PS_T_NotMove_FLT || PS_T_NotCO_FLT || PS_T_Alarm_FLT);
...
//ST_MID = (!CFG_MUK && !ST_DI_ON && !ST_DI_OF) || (CFG_MUK && DI_ON && DI_OF);
ST_MID = !ST_DI_ON && !ST_DI_OF;
// Открыт по алгоритму
 if (!ST_RUs && AVT_ON/* && DO_ON*/)
	{
	ST_AVT_ON = 1;
	ST_AVT_OF = 0;
	ST_BTN_ON = 0;
...
//Статусное слово положения ИМ
ST_POS= ST_DI_OF + (2.0 * ST_DI_ON);
