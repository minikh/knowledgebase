...
#define SZ_set	   BIT_SET_M(3412) //Открыть стендовую заслонку
#define SZ_clr	   BIT_CLR_M(3412) //Закрыть стендовую заслонку
#define SZ_tst	   BIT_TST_M(3412) //Закрыть стендовую заслонку
#define VZ_set	   BIT_SET_M(3413) //Открыть воздушную заслонку
#define VZ_clr	   BIT_CLR_M(3413) //Закрыть воздушную заслонку
#define VZ_tst	   BIT_TST_M(3413) //Проверить положение воздушной заслонки
...
//      else YdusMain = YdusMain -integStep*DUS_RATE;
	   ...
	   if ((Nvd0<8740) && !((DG_POZ<10) && !VZ_tst && !SZ_tst)) {
		   if (!StartTrigger) {
			StartTrigger = 1;
			}
