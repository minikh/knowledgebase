...
#define N_StrFStimer_out	RF(1565)				//Таймер ВО при обнаружении стружки в ФС через 180 мин, для отображения на пульте оператора
#define N_StrKPtimer_out	RF(1567)				//Таймер ВО при обнаружении стружки в КП через 180 мин, для отображения на пульте оператора
...
#define P_StrSPK_set     	BIT_SET_M(3639)			//"Стружка в линии откачки масла от ШПК"
...
#define N_TinBrkRezhim_set  BIT_SET_M(3664)			//"Отказ Твх на режиме"
#define N_TinBrkRezhim_clr 	BIT_CLR_M(3664)			
#define N_TinBrkRezhim_tst	BIT_TST_M(3664)
...
static float    Tin_old, Tvh_timer2, taimer_tvh, taimer_tvh_hour;   //Переменные для алгоритма Твх
...
static float    dTst, dTst1, dTst2, dTst3, dTst4, dTst5; // Скорость роста температуры на термопаре Тст
...
// формирование по отказу Твх - НО на режиме
	if ((abs(CT_IF - Tin_old)>= 1.0)||(P_TvhBrkDop_tst)||(FCT_IF)) taimer_tvh =0.0;  
		else if (taimer_tvh < 60.0) taimer_tvh +=cycle;
  
	Tin_old = CT_IF; // сохраняем Твх на предыдущем шаге

	if (taimer_tvh < 60.0) taimer_tvh_hour +=cycle;
	if (taimer_tvh_hour>3600.0) taimer_tvh_hour=3600.0;
	if (taimer_tvh >= 60.0) taimer_tvh_hour=0.0;
    
    // формирование нормального останова при отказе канала Твх в течении 60 мин
	if (taimer_tvh_hour == 3600.0) N_TinBrkRezhim_set;
		else if (externDeblock) N_TinBrkRezhim_clr;
    
	Tvh_timer2 +=cycle;
    
  // фильтрация и расчет Твх
    if (taimer_tvh >= 60.0)
	{
      if (Tvh_timer2 > 3.0)
      {
    if (((!P_TvhBrkDop_tst)&&(!FCT_IF)||(condition==6)||(condition==1)))
        {
        Tvh=(Tvh1 + Tvh2 + Tvh3 + Tvh4 + Tvh5 + Tvh6 + Tvh7 + Tvh8 + Tvh9 + Tvh10)/10;
        Tvh10=Tvh9;
        Tvh9=Tvh8;
        Tvh8=Tvh7;
        Tvh7=Tvh6;
...
//расчет приведенных оборотов ГГ	
...
//Контроль давления воздуха в разгрузочной полости CТ
	PvrpST=PizbST + 0.1;	
	if ((nvd>10400)||check)
		{if ((PvrpST/Pkvd)<0.25) P_osSTHigh_set; 							//Повышенная осевая сила ротора СТ
			else P_osSTHigh_clr;}
	  else P_osSTHigh_clr;
...
//Контроль наличия стружки ИЗМЕНЕНО В СООТВЕТСТВИИ С РЕШЕНИЕМ № 2009-313
	...
	if (StrSPK_tst) P_StrSPKtimer += cycle;else P_StrSPKtimer  = 0.0;//"Стружка в линии откачки масла от ШПК"  
...
//Контроль обратной раскрутки свободной турбины
	...
	if (A_ObRaskrST_timer > 0.5) A_ObRaskrST_timer = 0.5;
		if ((nvd<=1600)&&(nst>=300)&&(A_ObRaskrST_timer == 0.5)&&(!HP_tst)&&(!endHP_tst)&&(!R_promGo_tst)&&(!R_promEnd_tst)) A_ObRaskrST_set;		//"Обратная раскрутка ротора СТ"
...
//Контроль времени выбега ГГ
	...
	if ((!(SZ_tst || VZ_tst)) && (nvd < 100) && (nvd > 80) && (((VibegGGTimer < 100) && (!endHP_tst)&&(!R_promEnd_tst)) || ((VibegGGTimer < 80) && (endHP_tst)&&(!R_promEnd_tst)))) P_VibegGGMin_set;
		 
//Контроль времени выбега СТ
	if ((!(SZ_tst || VZ_tst)) && (nst < 1100) && (nst > 100))	VibegSTTimer+=cycle;								//"Мало время выбега ротора СТ"
	if ((!(SZ_tst || VZ_tst))&&(!endHP_tst)&&(!R_promEnd_tst) && (nst < 100) && (nst > 80) && (VibegSTTimer < 50)) P_VibegSTMin_set;
...
// Неисправность СТ
	if ((nvd>=NggSTBrk)&&((dNst <= -75.0) ||A_NstBrk_tst)&&(dTst >= 50.0)) NstFaultTimer+=cycle;
		else NstFaultTimer=0.0;
	if (NstFaultTimer > NstFaultTime) NstFaultTimer = NstFaultTime;
	if (NstFaultTimer >= NstFaultTime)  A_STbrk_set;
...
//Сборка требований НО
	if (N_TtHigh_tst||N_PtgHigh_tst||N_PmvhDmin_tst||N_PsufMax_tst||N_deltaPvxMax_tst||N_TmspKvdMax_tst||N_TmrpTvdMax_tst||N_TstHigh_tst||N_VibPpMax_tst||N_VibZpMax_tst||N_VibDMax_tst|| 
		(N_StrKP_tst && (N_StrKPtimer == 10800.0))||N_TmopStMax_tst||N_StrRPT_tst||N_TtBrk_tst||(N_StrFS_tst && (N_StrFStimer == 10800.0))||N_TstBrk_tst||N_TinBrkRezhim_tst||externNO) N_common_set;
...
//Сборка аварий!
	if (A_NvdBrk_tst||A_NvdDistort_tst||A_Ttzap_tst||A_Pompaz_tst||A_NstBrk_tst||A_NstDistort_tst||A_TtTtzapBrk_tst||
		...
		A_DozBrk_tst||A_VNABrk_Neg_tst||A_VNABrk_Pos_tst||A_DozBrkDop_tst||A_VNABrkDop_tst||A_PtgMax_tst||A_PmvhDno_tst||
		A_PtgMin_tst||A_PgInStVmax_tst||A_PgInStVmin_tst||A_PgInStVmin_proval_tst||A_PmvhDmin_tst||A_TmspKvdMax_tst||
		A_TmrpTvdMax_tst||A_TmopStMax_tst||A_RateVibPpMax_tst||A_RateVibZpMax_tst||A_STbrk_tst||A_CVStVMax_tst||A_StrMStV_tst||
		A_PoutStVMax_tst||A_aoBZD_tst||externAO||(A_SKBrk_tst&&(condition>1)&&(!HP_tst)&&(condition!=6))||
		(A_KPVIMBrk_tst&&(condition>1)&&(!HP_tst)&&(condition!=6))||(A_KPGIMBrk_tst&&(condition>1)&&(!HP_tst)&&(condition!=6))) A_common_set;
