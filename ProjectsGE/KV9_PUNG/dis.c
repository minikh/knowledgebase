...
#define R_kt_Tst_set		BIT_SET_M(3484)			//*Установка флага "Контур ограничения Tst"
#define R_kt_Tst_clr		BIT_CLR_M(3484)			//*Сброс флага "Контур ограничения Tst"
...
#define Nstart_set			BIT_SET_M(3412)			//*Включить электростартер
#define Nstart_clr			BIT_CLR_M(3412)			//*Сброс команды - Включить электростартер
#define Nstart_tst			BIT_TST_M(3412)			//*Проверка команды на Включение электростартера

#define PNstart_set			BIT_SET_M(3413)			//*Включить повышенную мощность электростартера
#define PNstart_clr			BIT_CLR_M(3413)			//*Сброс команды - Включить повышенную мощность электростартера
#define PNstart_tst			BIT_TST_M(3413)			//*Проверка команды на Включение повышенной мощности электростартера
...
if (Ptopl < 1.5) Ptopl = 1.5;
	else if (Ptopl > 3.5) Ptopl = 3.5;
setDoz = harakt(IoutDoz, Gt_tek/(PtgHarDoz/Ptopl));
if (setDoz<0) setDoz = 0;
if (setDoz>100) setDoz = 100;
} 
...
if ((Reg_GtPk_ogr_u >= -1.5)&&(Reg_GtPk_ogr_u <= 10.5)) 			Reg_GtPk_ogr = Reg_GtPk_ogr_u;		//Регулировка величины ограничения Gт/Рк (параллельное смещение программы по оси Gт/Рк)
...
if (startTimer >= 350.0) TimeZapHigh_set;
...
//		 Kp_Tt = KpTt;
     ...
     case CND_RATE:
		...
		if (longZAP)
		 ...
		 if ((Starter_Old == 1) && !(VZ_tst || Nstart_tst)) IntegNvd = nvd;
	 ...
	 case CND_STAB:
		 ...
		 if (((!up)&&(!stabST_on))||(((nst_ust_min - nst) <0)&&(stabST_on)) || NO_tst) {
			...
			//		 Out_sel1 = dNgg_DOWN;
					 Metka = kt_down;//}}
			 if (!(down||downDI) && !NO_tst && !stabST_on && (Out_sel1 < (nvd_ust*CTvhNvd - IntegNvd)*Kdot)) {
...
if (VZ_tst || Nstart_tst) Starter_Old = 1;
	else Starter_Old = 0;
