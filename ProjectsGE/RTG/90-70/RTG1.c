#include "math.h"     // математические функции
#include "plcc9070.h" //процедуры для контроллера

//Входы
#define  IN_Ro_s			RF(5001)	//Плотность газа при стандартных условиях, кг/м3				0.6773418
#define  IN_X_a				RF(5003)	//Молярная доза азота, %										0.79
#define  IN_X_u				RF(5005)	//Молярная доза углекистлоты, %									0.069
#define  IN_Dcu_20			RF(5007)	//Диаметр отверстия СУ при стандартных условиях, м				0.03722
#define  IN_D_20			RF(5009)	//Диаметр трубы при стандартных условиях, м						0.08213
#define  IN_R_sh			RF(5011)	//Эквивалентная шероховатость ИТ, м								0.00025
#define  IN_Alfa_cu			RF(5013)	//Температурный кэффициент линейного расширения материала СУ	0.00001634
#define  IN_Alfa_t			RF(5015)	//Температурный кэффициент линейного расширения материала ИТ	0.00001125
#define  IN_r_n				RF(5017)	//Начальный радиус входной кромки диафрагмы, м					0.00004
#define  IN_Tau_t			RF(5019)	//Текущее время эксплуатации диафрагмы, год						0.5
#define  IN_t				RF(5021)	//Температура газа, грС											43.5
#define  IN_P_iz			RF(5023)	//Избыточное давление, Па										2840000.0
#define  IN_P_at			RF(5025)	//Атмосферное давление, Па										100000.0
#define  IN_dP				RF(5027)	//Перепад на СУ, Па												27200.0
#define  IN_ContractTime	RI(5029)	//Контрактное время												10
#define  IN_dP_F			RI(5030)	//Отказ замера перепада давления на диафрагме					false


//Выходы
#define  raschet_q_c			RF(5031)	//Объемный расход среды прив. к ст. усл., м3/сек	1.010234
#define  raschet_q_v			RF(5033)	//Объемный расход среды при рабочих условиях, м3/с	0.0364177
#define  raschet_q_m			RF(5035)	//Массовый расход, кг/с								0.6842738
#define  raschet_Error			RI(5037)	//Fault												0
#define  raschet_Fault			RI(5038)	//Ошибки вычисления расхда (по битам)				0

//int GefMain( unsigned int *in, float *res)
main()
{
float Beta, K_cu, Dcu, K_t, D, E, a, r_k, K_p, P, P_MPa, TT, Z_c, P_pk, T_pk, Z, P_a, T_a, dT_a, F, Tu_1;
float Tu_0, B_1, B_0, B_2, K, Ro, P_p, T_p, denominator, Mu_t, C_mu, Mu, K_ad, ee, idint, L_1, L_2, Re;
float M_2, R_a, c1, c2, A0, A1, A2, M_1, aa, c, R_amax, R_amin, A_sh, k_D, k_R, exp1, exp2, exp3, Lam1, Lam2, K_sh;
float prom1, prom2, prom3, prom4, prom5, prom6, prom7, prom8, prom9, prom10, prom11, prom12, prom13, prom14;
float prom15, prom16, prom17, prom18, prom19, prom20, prom22, prom23, prom24;
float A_0[16], A_1[16], A_2[16];
int i, j, prom21;


//Сброс ошибок
   raschet_Error=0;
   raschet_Fault=0;

// Исходные данные 

//1 Диаметр отверстия диафрагмы при температуре 20 °С, м. 
//  IN_Dcu_20

// ограничение по диаметру отверстия СУ
if ( IN_Dcu_20 < 0.0125) 
	 {
	  IN_Dcu_20 = 0.0125;
	  raschet_Error |= (1<<1);//Неверное значение диаметра диафрагмы (СУ)
	 }
else if  ( IN_Dcu_20 > 0.09)
	 {
	  IN_Dcu_20 = 0.09;
	  raschet_Error |= (1<<1);//Неверное значение диаметра диафрагмы (СУ)
	 }



//2 Внутренний диаметр ИТ при температуре 20 °С, м. 
//  IN_D_20

//Ограничение по диаметру Измерительного трубопровода
if  ( IN_D_20 < 0.05)
	 {
	  IN_D_20=0.05;
	  raschet_Error |= (1<<2);//Неверное значение диаметра измерительного трубопровода
	 }
else if  ( IN_D_20 > 1.0)
	{
	 IN_D_20 = 1.0;
	 raschet_Error |= (1<<2);//Неверное значение диаметра измерительного трубопровода
	}


//3 Среднее арифметическое отклонение профиля шероховатости ИТ 
//  IN_R_sh

// ограничение по R_sh
if (( IN_R_sh < 0.0) || ( IN_R_sh > 0.01))
	 {
	  IN_R_sh = 0.0003;
	  raschet_Error |= (1<<3);//Не верное значение среднее арифметическое отклонение профиля шероховатости ИТ
	 }
 

//4 Материал, из которого изготовлена диафрагма. Температурный коэффициент линейного расширения материала СУ
//  IN_Alfa_cu 

//ограничение по коэф. теплового расширения металла СУ (диафрагмы) 
if (( IN_Alfa_cu < 0.0) || ( IN_Alfa_cu>0.0005))
 {
  IN_Alfa_cu=0.0000184;
  raschet_Error |= (1<<4); //Не верное значение коэф. теплового расширения металла СУ (диафрагмы)
 }


//5 Материал, из которого изготовлен ИТ.Температурный коэффициент линейного расширения материала ИТ
//   IN_Alfa_t

//ограничение по коэф. теплового расширения металла ИТ (измерительного трубопровода)
if (( IN_Alfa_t < 0.0) || ( IN_Alfa_t>0.0005))
	{
	 IN_Alfa_t=0.0000184;
	 raschet_Error |= (1<<5); //Не верное значение коэф. теплового расширения металла ИТ (измерительного трубопровода)
	}


//6 Начальный радиус входной кромки диафрагмы, м
//  IN_r_n

// ограничение по r_n
if (( IN_r_n < 0.0) || ( IN_r_n>0.01))
	{
	 IN_r_n=0.00004;
	 raschet_Error |= (1<<6);//Не верное значение начального радиуса входной кромки диафрагмы
	}


//7 Текущее время   эксплуатации диафрагмы с момента определения значения начального радиуса входной кромки диафрагмы, год
//  IN_Tau_t

if (( IN_Tau_t < 0.0) || ( IN_Tau_t > 1.0))
	{ 
	 IN_Tau_t = 0.0;
	 raschet_Error |= (1<<7); //Не верное значение тек времени эксл диафрагмы с момента определения значения начальн. радиуса входной кромки
	}


//8 Содержание углекислого газа в природном газе
//  IN_X_u
// ограничение по Xu
if (( IN_X_u < 0.0) || ( IN_X_u>0.16))
	{ 
	 IN_X_u=0.0147;
	 raschet_Error |= (1<<8);//Не верное значение содержания углекислого газа в природном газе
	}


//9 Содержание азота в природном газе
//  IN_X_a
// ограничение по Xa
if ((  IN_X_a < 0.0) || (  IN_X_a>0.16))
	{
	 IN_X_a=0.0088;
	 raschet_Error |= (1<<9);// Не верное значение содержания азота в природном газе
	}



//10 Плотность природного газа при стандартных условиях, кг/м^3 
//  IN_Ro_s
// ограничение по плотностю газа
if ((  IN_Ro_s<0.6) || (  IN_Ro_s>0.8))
	{
	 IN_Ro_s=0.735;
	 raschet_Error |= (1<<10);//Не верное значение плотности природного газа при стандартных условиях
	}


//11 Относительная влажность природного газа, %

//12 Перепад давления на диафрагме, Па
//  IN_dP
	if ( IN_dP_F)  raschet_Error |= (1<<11);


//13 Избыточное давление, Па
//  IN_P_iz
if  ( IN_P_iz < 0.0)  IN_P_iz = 0.0;


//14 Атмосферное давление, Па
//  IN_P_at

if  ( IN_P_at > 140000.0)  IN_P_at = 140000.0;
else if  ( IN_P_at < 70000.0)  IN_P_at = 70000.0;

//15 Температура природного газа, °С
//  IN_t

//Контрактное время
if  ( IN_ContractTime < 0)  IN_ContractTime = 0;
else if  ( IN_ContractTime> 23)  IN_ContractTime = 23;



//Проверка условий возможности вычисления расхода
if  ( IN_P_iz <150000.0 ||   IN_P_iz > 4000000.0 
	||    IN_t < -20.0 ||   IN_t > 80.0 
	||   IN_dP < 20.0 ||   IN_dP > 100000.0 ||   IN_dP_F)   raschet_Fault =1;




if ( raschet_Fault == 0) 
 {
	//ГОСТ 8.586-2005
	//GAV
	//1 Коэффициент, учитывающий изменение диаметра отверстия диафрагмы, вызванное отклонением температуры природного газа от 20 °С
	//ГОСТ 8.586.1 формула (5.6)
	K_cu = 1.0 +   IN_Alfa_cu * (  IN_t - 20.0);

	//2 Диаметр отверстия диафрагмы при рабочей температуре, м
	//ГОСТ 8.586.1 формула (5.4)
	Dcu =    IN_Dcu_20 * K_cu;
	//3 Коэффициент, учитывающий изменение диаметра ИТ, вызванное отклонением температуры природного газа от 20 °С
	//ГОСТ 8.586.1 формула (5.7)
	K_t = 1.0 +   IN_Alfa_t * (  IN_t - 20.0);
	//4 Внутренний диаметр ИТ при рабочей температуре природного газа, м
	//ГОСТ 8.586.1 [формула (5.5)]
	D =     IN_D_20 * K_t;
	// 5 Относительный диаметр отверстия диафрагмы Beta
	//ГОСТ 8.586.1 [формула (3.1)]

	//Огрнничение по отношению диаметров СУ и ИТ
	//ограничения по Beta
	Beta = Dcu/D;
	if (Beta < 0.1) 
		{
		Beta = 0.1;
		 raschet_Error |= (1<<12);//Не допустимое отношение диаметра диафрагмы (СУ) к диаметру ИТ
		}
	else if (Beta > 0.75) 
		{  
	  	Beta =0.75;
		 raschet_Error |= (1<<12);//Не допустимое отношение диаметра диафрагмы (СУ) к диаметру ИТ
		}
	

	//6 Коэффициент скорости входа
	//     8.586.1 [        (3.6)] E=1 / sqrt(1-(Beta ^ 4));
	//prom3 = 1.0-(Beta ^ 4.0);
	prom3 = 1.0-(pow(Beta, 4.0));
	prom4 = sqrt(prom3);
	E=1.0 /prom4 ;
	//7 Поправочный коэффициент, учитывающий притупление входной кромки диафрагмы K_p
	//ГОСТ 8.586.2 [формула (5.13)]
	a=0.000195;//  - параметр, учитывающий тип измеряемой среды, который принимают равным 0,00019 для жидкости 0,000195  для газа и 0,0002  для пара;
	//r_k = a - (a -   IN_r_n) * 2.718282 ^ (-  IN_Tau_t / 3.0);//радиус входной кромки диафрагмы
	r_k = a - (a -   IN_r_n) * pow(2.718282, (-  IN_Tau_t / 3.0));//радиус входной кромки диафрагмы
	if (r_k < 0.0004 * D) K_p = 1.0;
	else K_p = 0.9826 + pow((r_k / Dcu + 0.0007773), 0.6);
	

	//8 Абсолютное давление природного газа перед диафрагмой, Па
	//ГОСТ 8.586.5 [формула (6.2)]
	//
	P =   IN_P_at +   IN_P_iz;
	P_MPa = P/1000000.0 ;//Для расчетов по ГОСТ 30319


	//9 Термодинамическая температура природного газа, K
	//ГОСТ 8.586.5 [формула (6.3)]
	//
	//ограничение по Т газа
	if  ( IN_t < -50.0)
		 {
		  IN_t = -50.0;
		  raschet_Error |= (1<<13);
		 }
	else if  ( IN_t > 150.0) 
		 {
		  IN_t = 150.0;
		  raschet_Error |= (1<<13);
		 }
	

	TT =   IN_t + 273.15;


	//10 Фактор сжимаемости природного газа при стандартных условиях
	//и известных плотности природного газа при стандартных условиях и содержании в нем азота и диоксида углерода, т.е. по формуле
	//ГОСТ 30319.1 [формула (24)] 
	// ВНИМАНИЕ в ГОСТ 30319 в расчетах давление измеряется в МПа
	//Z_c
	Z_c = 1.0 - pow((0.0741 *   IN_Ro_s - 0.006 - 0.063 *   IN_X_a - 0.0575 *   IN_X_u), 2.0);
	//11 Фактор сжимаемости природного газа при рабочих условиях
	//ГОСТ 30319.2 [формула (6)]
	//Z
	P_pk= 2.9585 * (1.608 - 0.05994 *   IN_Ro_s +   IN_X_u - 0.392 *   IN_X_a);//МПа
	T_pk= 88.25*(0.9915+1.759*  IN_Ro_s-   IN_X_u-1.681*   IN_X_a);

	if (P_pk <= 0.0 || T_pk <= 0.0)
		 {
		  raschet_Error |= (1<<14);//Ошибка в расчете фактора сжимаемости при рабочих условиях
		 Z= Z_c;//Приравниваем фактор сжимемости  при рабочих условиях к значению при нормальных условиях
		 }
	else
		{
		P_a= 0.6714 * (P_MPa / P_pk) + 0.0147;//МПа
		T_a= 0.71892*(TT/T_pk)+0.0007;
		dT_a = T_a - 1.09;
		// ^  ^  ^  ^  ^  ^  ^  ^  ^  ^  ^  ^ 
		//Ограничение по dT_a
		if (dT_a > 0.3) dT_a =0.3;
		else if (dT_a<-0.25) dT_a = -0.25;
		// ^  ^  ^  ^  ^  ^  ^  ^  ^  ^  ^ *

		if (P_a >= 0.0 && P_a <= 2.0 && dT_a >= 0.0 && dT_a <= 0.3) 
			F=(75.0 * pow(10.0,(-5)) * pow(P_a,2.3)) / pow(2.718282,(20.0 * dT_a)) + 11.0 * pow(10.0,-4) * pow(dT_a,0.5)* (P_a * pow(2.17 - P_a + 1.4 * pow(dT_a, 0.5), 2.0));
		
		if (P_a >= 0.0 && P_a < 1.3 && dT_a >= -0.25 && dT_a < 0.0) 
			F=(75.0 * pow(10.0 , -5) * pow(P_a , 2.3) * (2.0 - pow(2.718282 , (20.0 * dT_a))) + 1.317 * P_a * (1.69 - pow(P_a , 2))* pow(dT_a , 4));
		
		if (P_a >= 1.3 && P_a < 2.0 && dT_a >= -0.25 && dT_a < 0.0)  
			F=(75.0 * pow(10.0 , -5) * pow(P_a , 2.3) * (2.0 - pow(2.718282 , (20.0 * dT_a))) + 0.455 * P_a * (1.3 - P_a) * (1.69 * pow(2.0 , 1.25) - pow(P_a , 2)) * (dT_a * (0.03249 + 18.028 * pow(dT_a ,  2)) + pow(dT_a , 2) * (2.0167 + pow(dT_a , 2) * (42.844 + 200.0 * pow(dT_a , 2))))); 
		

		Tu_1= pow(T_a , 5) / (pow(T_a , 2) * (6.60756 * T_a - 4.42646) + 3.22706);
		Tu_0=((pow(T_a , 2) * (1.77218 - 0.8879 * T_a) + 0.305131) * Tu_1) /pow(T_a , 4);

		B_1=(2.0 * Tu_1 / 3.0) - pow(Tu_0 , 2);
		B_0=Tu_0 * (Tu_1 - pow(Tu_0 , 2)) + 0.1 * Tu_1 * P_a * (F - 1.0);
		B_2 = pow(B_0 + pow(pow(B_0 , 2)+ pow(B_1 , 3) , 0.5) , 0.333333);


		Z=((pow(1.0 + (0.00132) / pow(T_a , 3.25) , 2) / (B_1 / B_2 - B_2 + Tu_0)) * (P_a / 10.0));
	}

	//12 Коэффициент сжимаемости природного газа
	//ГОСТ 30319.1 [формула (7)]
	//K
	K= Z/Z_c;
	//13 Плотность природного газа, кг/м 
	//ГОСТ 30319.1 [формула (6)] Па•с
	//Плотность в рабочих условиях (Стандартные условия t =293.15 град К и Р = 0.101325 МПа)

	Ro=  IN_Ro_s * P_MPa * 293.15/ (0.101325 * TT * K);


	//14 Динамическая вязкость природного газа
	//ГОСТ 30319.1 [формулы (44) и (45)]
	//Псевдокритические давления (МПа) и температура ГОСТ 30319.2 формулы 17 и 18 



	if  ( raschet_Error & (1<<14)) // raschet_Error.X[14] = 1 then //*******************************************************************
		   raschet_Error |= (1<<15); //Ошибка в расчете динамической вязкость природного газа
	else
		{
		//Привденные давление и температура
		P_p=P_MPa/P_pk;
		T_p=TT/T_pk;
		//динамической вязкости природного газа и многих его компонентов, зависящую от температуры, при давлениях до 0,5 МПа , мкПа?с
		denominator = pow( IN_Ro_s , 0.5)+2.08-1.5*(  IN_X_a+  IN_X_u);
		if (denominator > 0.0)
			{
			Mu_t=3.24*(pow(TT , 0.5)+1.37-9.09* pow( IN_Ro_s , 0.125))/denominator;
				//Вязкость при повышенных давлениях (до 12 МПа) для природного газа вычисляют по формуле
				denominator = 30.0*(T_p-1.0);
				if (denominator != 0.0)
					{
					C_mu=1.0+(pow(P_p , 2)/ denominator);
					Mu=Mu_t*C_mu * 0.000001;//Пересчет в Па*с
					}
				else  raschet_Error |= (1<<15); //Ошибка в расчете динамической вязкость природного газа
			}
		else  raschet_Error |= (1<<15); //Ошибка в расчете динамической вязкость природного газа
		
		}

	if   ( raschet_Error & (1<<15))// raschet_Error.X[15] = 1 then   //********************************************************************
		Mu= 0.0000111315;// Fallback при ошибке в расчетах динамической вязкости
	
	//15 Показатель адиабаты природного газа
	//ГОСТ 30319.1 [формула (28)]

	K_ad=1.556 * (1.0 + 0.074 *   IN_X_a) - 3.9 * pow(10.0 , -4) * TT * (1.0 - 0.68 *   IN_X_a) - 0.208 *   IN_Ro_s + pow((P_MPa / TT) , 1.43) * (384.0 * (1.0 -   IN_X_a) * pow((P_MPa / TT), 0.8) + 26.4 *   IN_X_a);


	//16 Коэффициент расширения
	//ГОСТ 8.586.2 [формула (5.7)]
	if  ( IN_dP/P > 0.025 && K_ad == 0.0)
		{
		 raschet_Error |= (1<<16); //Ошибка в расчете Коэффициент расширения
		ee= 1.0; //Fallback для коэффициента расширения
		}
	else
		ee= 1.0 - (0.351 + 0.256 * pow(Beta , 4) + 0.93 * pow(Beta , 8)) * (    1.0 - pow((1.0 -   IN_dP / P) , (1.0 / K_ad))   );
	



	//Расчет расхода природного газа,
	 c2= 1.0;
	 c1= 0.0;
	idint =0.0;

	L_1=0.0; // углового способа отбора  давлений
	L_2=0.0;

	if (D < 0.07112) M_2=0.011*(0.75-Beta)*(2.8-D/0.0254);
	
	if (D >= 0.07112) M_2=0.0;
	

	///1 Начальное значение числа Рейнольдса
	Re = 1000000.0;
	
	// Массивы коээфициентов, используемые при расчете поправочного коэффициента, учитывающего шероховатость внутренней поверхности ИТ
	A_0[0]=8.87; A_0[1]=-3.7114; A_0[2]=0.41841; A_0[3]=0.0; A_0[4]=27.23; A_0[5]=-11.458; 
	A_0[6]=1.6117; A_0[7]=-0.07567; A_0[8]=16.5416; A_0[9]=-6.60709; A_0[10]=0.88147; A_0[11]=-0.039226;
	A_1[0]=6.7307; A_1[1]=-5.5844; A_1[2]=0.732485; A_1[3]=0.0; A_1[4]=-25.928; A_1[5]=12.426; 
	A_1[6]=-2.09397; A_1[7]=0.106143; A_1[8]=322.594; A_1[9]=-132.2; A_1[10]=17.795; A_1[11]=-0.799765;
	A_2[0]=-10.244; A_2[1]=5.7094; A_2[2]=0.76477; A_2[3]=0.0; A_2[4]=1.7622; A_2[5]=-3.8765; A_2[6]=1.05567;
	A_2[7]=-0.076764; A_2[8]=-92.029; A_2[9]=37.935; A_2[10]=-5.1885; A_2[11]=0.23583;

	R_a=  IN_R_sh/3.141592;
	
	prom2 = (( c2 -  c1) /  c2);
	
	prom1 = abs(prom2);
	
while  (prom1 > 0.00001 && idint <10.0) //Ограничение по точности и кол-ву итераций
	{
		idint = idint +1.0;

	// Расчет Коэффициент истечения
	//ГОСТ 8.586.2 [формула (5.6)]
		A0=0.0;
		A1=0.0;
		A2=0.0;
		 c1=  c2;
		M_1= 2.0 * L_2 / (1.0 - Beta);
		aa= pow((19000.0 * Beta / Re) , 0.8);
	//2 Коэффициент истечения
	//ГОСТ 8.586.2 [формула (5.6)]
		c=0.5961 + 0.026 * pow(Beta , 2) - 0.216 * pow(Beta , 8)+0.000521* pow((pow(10.0 , 6) * Beta / Re) , 0.7)+ 	(0.0188+0.0063*aa)*pow(Beta , 3.5) * pow((pow(10.0 , 6)/Re) , 0.3)+(0.043+0.08* pow(2.718282 , (-10.0 * L_1)) - 0.123 * pow(2.718282 , (-7.0*L_1))) * (1.0 -0.11*aa)*pow(Beta , 4)/(1.0 -pow(Beta , 4))-0.031 * (M_1 - 0.8 * pow(M_1 , 1.1)) * pow(Beta , 1.3) + M_2;
	// 3 Поправочный коэффициент, учитывающий шероховатость внутренней поверхности ИТ
	//ГОСТ 8.586.2 [формула (5.11)]
		//Поправочный коэф. на шероховатость ИТ ф.5.11 ГОСТ 8.586.2-2005

	// Определяем Ra_max
		j=0;
		if (Re > pow(10.0 , 4) && Re <= pow(10.0 , 5)) j= 0;
		
		if (Re > pow(10.0 , 5) && Re <= 3.0* pow(10.0 , 6)) j= 4;
		
		if (Re > 3.0* pow(10.0 , 6) && Re <= pow(10.0 , 8)) j= 8;
		
		for (i= 0; i<4; i++) //; i != 4; i++)
			{
			prom20 = log10(Re); //log10_REAL(Re, );
			prom5 = pow(prom20 , i);
			prom21 = i+j;
			A0=A0 + A_0[prom21] *(prom5);
			A1=A1 + A_1[prom21] *(prom5);
			A2=A2 + A_2[prom21] *(prom5);
			}
		
		if (Re <= pow(10.0 , 4)) R_amax= 0.718866 * pow(Beta , -3.887) + 0.364;
		
		if (Re > pow(10.0 , 4) && Beta < 0.65) R_amax=A0 * pow(Beta , A1) + A2;
		
		if (Re > pow(10.0 , 4) && Beta >= 0.65) R_amax=A0 * pow(0.65 , A1) + A2;
		


		if (R_amax >= 15.0) R_amax=(15.0* pow(10.0 , -4)) * D;
		else R_amax = R_amax * D * 0.0001;

		

	// Определяем Ra_min

		prom6 = log10(Re);
		if (Beta < 0.65) R_amin=7.1592 - 12.387 * Beta - (2.0118 - 3.469 * Beta) * prom6 + (0.1382 - 0.23762 * Beta) * pow(prom6 , 2);
		
		if (Beta >= 0.65) R_amin=-0.892353 + 0.24308 * prom6 - 0.0162562 * pow(prom6 , 2);
		
		if (R_amin <= 0.0 || Re < 3.0* pow(10.0 , 6)) R_amin= 0.0;
		
		//	R_amin

		A_sh= 3.141592 *R_a;               
		k_D= (0.26954 * 3.141592 * R_a) / D; 
		k_R= 5.035 / Re;      
		
		prom7 = k_D + 3.3333 * k_R;	
		prom8 = log10(prom7);
		exp1 = k_R * prom8;

		if (exp1 > k_D) 
			   raschet_Error |= (1<<17); //Oшибка в расчете поправочного коэффициента, учитывающий шероховатость внутренней поверхности ИТ
		else
			{
			prom9 = k_D - exp1;
			prom10 = log10(prom9);
			exp2 = (2.0 * A_sh / D) - (37.36 * prom10 / Re);
			if (exp2 < 0.0)
				{
				 raschet_Error=1;//Oшибка в расчете поправочного коэффициента, учитывающий шероховатость внутренней поверхности ИТ
				Lam1= 0.0001;
				}
			else
				{
				prom11 = log10(exp2);
				Lam1= pow((1.74 - 2.0 * prom11) , -2);
   	   
				if (R_a > R_amax) 
					{
					A_sh= 3.141592*R_amax;
					k_D= 0.26954 *3.141592* R_amax/D;
					}
				
				if (R_a < R_amin) 
					{
					A_sh=3.141592*R_amin;
					k_D= 0.26954 * 3.141592 * R_amin/D;
					}
				}
			
			prom12 = (k_D + 3.3333 * k_R);
			prom13 = log10 (prom12);
			exp1 = k_R * prom13;
			if (exp1 > k_D)
				   raschet_Error |= (1<<17); //Oшибка в расчете поправочного коэффициента, учитывающий шероховатость внутренней поверхности ИТ
			else
				{
				prom14 =(k_D - exp1);
				prom15 = log10(prom14);
				exp2 = (2.0 * A_sh / D) - (37.36 * prom15/ Re);
				if (exp2 < 0.0) 
					{
					 raschet_Error |= (1<<17); //Oшибка в расчете поправочного коэффициента, учитывающий шероховатость внутренней поверхности ИТ
					Lam2= 0.0001;
					}
				else
					{
					prom16 = log10(exp2);
					Lam2= pow((1.74 - 2.0 * prom16) , -2);
					}
				}
			}
	// находдим Кш наконец-то!!!

		if  ( raschet_Error & (1<<17)) // raschet_Error.X[17]=1 then //***********************************************************
			K_sh = 1.0;
		else
			K_sh=1.0 + 5.22 * pow(Beta , 3.5) * (Lam1 - Lam2);
		

	//4 Объемный расход природного газа, приведенный к стандартным условиям,  куб м /с
	//ГОСТ 8.586.5 [формула (5.8)]
		exp3 = 0.25 *3.141592 * pow(Dcu , 2) * c * E * K_sh * K_p * ee ;
		prom22 = 2.0 *   IN_dP * (( P * 293.15) / (  IN_Ro_s * 101325.0 *TT* K));
		prom17 = sqrt(prom22);
		 c2= exp3 * prom17;

	//5 Уточненное значение числа Рейнольдса
	//ГОСТ 8.586.5 [формула (5.11)]
		Re=(4.0 *  c2 *   IN_Ro_s) / (3.141592* D * Mu); 
	
	} //while  (prom1 > 0.00001 && idint <10.0) //Ограничение по точности и кол-ву итераций

	   raschet_q_c =  c2;
	  prom23 = 2.0 *   IN_dP *   IN_Ro_s * P * 293.15 / (101325.0 * TT * K);
	  prom18 = sqrt( prom23);
	   raschet_q_m = exp3 * prom18;
	  prom24=  2.0 *   IN_dP * 101325.0 * TT * K / (  IN_Ro_s * 293.15 *P );
	  prom19 = sqrt(prom24);
	   raschet_q_v = exp3 * prom19;
  
 }
else
	  {
	   raschet_q_c = 0.0;
	   raschet_q_m = 0.0;
	   raschet_q_v = 0.0;
	  }





	return(OK);
}
