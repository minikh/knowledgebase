...
short TON(short IN, int Num_TON, float t_cycle, long PT)
/*
IN - вход,
...
short R_TRIG(short signal, short* prev_cond)
{	
	if((!*prev_cond)&&(signal))
	{
		*prev_cond = signal;
 ...
 short F_TRIG(short signal, short* prev_cond)
{	
	if((prev_cond)&&(!signal))
	{
		*prev_cond = signal;
...
float ramp(short enable1, float ramp_rate, float y_target, float y_current, float delt_T )
{
	float dd;
	float dy;	
...
int CPID( DPID* pid)
 {
	static float err_prev_;
	static float PV_prev1_;
	static float PV_prev2_;
	float D_part;
	static short goal_dir_prev_;
	short DB_active;
	...
	if((pid->loop_enable)&&(pid->loop_valid))
	{
		...
		{     
			...
			pid->dI_part = pid->KI * (pid->err + err_prev_ )/2.0 * pid->delt_t ; 
		...
		if(DB_active)
		{
			...
			pid->goal_dir = 0 ; //(* increase output *)
		else
			pid->goal_dir = 1 ; //(* decrease output  *)
...
int limVer ( DSEL * s_sel, int* test_reg)
 {
	static short 	cond[8];
	int		i;
	short	LimCond_Q;
	int		num_lim;
	static short	prev_goal;
	int 	main_loop;
	...
	for(i = 1; i <= s_sel->N_loops; i++)// (* Verification all limit loops *)
	{
		...
		{		
			...
			if(((s_sel->pid_struct[i].loop_type == -1) && (cond[i] == 0)) || ((s_sel->pid_struct[i].loop_type == 1) && (cond[i])) || ((s_sel->pid_struct[i].loop_type == 2) && (s_sel->pid_struct[i].cur_act)))
			{
				s_sel->pid_struct[i].active = 1; //(* Limit active: PV low then lowSP or High then highSP, latched  *) 
				s_sel->pid_struct[i].cur_act = 1; //(* Limit active: PV low then lowSP or High then highSP, not latched *)
				s_sel->LimitCondition = 1 ;     //(* Latched *)	
				if(i == 2)test_reg[12] += 1;
			
			}
			else
				s_sel->pid_struct[i].cur_act = 0; // (* Limit was active:  *)  
		}	
		else if(i == main_loop)
		{
			s_sel->pid_struct[main_loop].cur_act = 1;
			s_sel->pid_struct[main_loop].active = 1;
		}
		else
		{
			s_sel->pid_struct[i].cur_act = 0;
			s_sel->pid_struct[i].active = 0;
		}
		...
		{
			num_lim = 0;
			s_sel->oppos_limits = 0;
		}
	} 
	return (1);
 }

 int stopLim(DSEL * s_sel, int* test_reg)
{

	short current_active;
	int main_loop;
	int i;
	static short current_active_previous;
	short cond_1;
	short cond_2;
	short cond_3;
	...
	{
		s_sel->LimitCondition = 0;
		s_sel->force_out_of_limit = 0;
		for(i = 1; i <= s_sel->N_loops; i++)
		{
			...
			if(i != main_loop)
			{
				s_sel->pid_struct[i].active = 0;   //(* unlatch all included limits *)
				s_sel->pid_struct[i].cur_act = 0; 
				if (i == 2)
				{
					test_reg[2] += 1;
...
int SIG_SEL( DSEL * s_sel, int* test_reg ) 
{
	int main_loop_in;
	static int loop_selected;
	short first_lim_in_man;
	int i;
	short cond_PplusD_selected;
	short cond_dI_selected;
	...
	if (s_sel->pid_struct[loop_selected].loop_enable == 0)  //(* check for previous scan enabled ?*) 
			s_sel->LimitCondition = 0;
...
/*
int GefMain(gt_Mstruct, test_reg)//, IN, t_cycle, PT,  Q, EN)
...
{
	//int ENO = 0;
	SIG_SEL(gt_Mstruct, test_reg);
	// float PT1 = (*PT) / 1000;
    //*Q = TON(*IN, ENO, *t_cycle, PT1);
	//*EN = TON_ARRAY[ENO];
