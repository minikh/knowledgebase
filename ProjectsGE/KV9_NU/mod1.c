...
if (init_tst)  {// инициализация
	...
	if (Nvd>100000)  Nvd =100000;
	if (Nvd0>100000)  Nvd0 =100000;
	if (Nst>100000)  Nst =100000;
} 
...
//      else YdusMain = YdusMain -integStep*DUS_RATE;
	   ...
	   if (Nvd0>8750) { 
        if (StartTrigger) StartTrigger=0; // переход от стартовой модели к рабочей
	   }
	...
	//RF(20001) = Nvd0;
	//RF(20003) = ZsVSToNvd0func;
	//RF(20005) = Mst*MstToNvdfunc;
	//RF(20007) = cycle*(ZsVSToNvd0func-Mst*MstToNvdfunc+Gt0t*Gt0startToNvd0func)*1000.0*(30.0/M_PI)*(30.0/M_PI)/(12.17*Nvd0);
    //RF(20009) = Gt0t*Gt0startToNvd0func;
	}
...
//RF(10047) = cycle*30000.0*(-POW_COEFF*Nst*Nst)/((J_ST+J_N)*M_PI*Nst);
	...
	if (Gt0t>300) {
		if (AZ1_tst || EnginWorks_tst)  
			//Tt=(((Nvd0ToTst0func+Nvd0ToA3func*(Gt0t-Gt0n)/Gt0n)*(Tatm)/NORM_T+(Fvna-set_VNA)*Nvd0ToA6func)/4)+280;
			Tt=(((Nvd0ToTst0func+Nvd0ToA3func*(Gt0t-Gt0n)/Gt0n)*(Tatm)/NORM_T+(Fvna-set_VNA)*Nvd0ToA6func)/3)+380;
		}
    else Tt=Tatm + 160.0;
