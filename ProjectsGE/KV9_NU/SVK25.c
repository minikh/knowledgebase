...
#define deltaPvx			RF(1081)				//Перепад между атмосферным давлением и полным давлением воздуха на входе в компрессор КНД  0..6.3 кПа 
#define deltaPvxst			RF(1083)				//Перепад между атмосферным давлением и статическим давлением воздуха на входе в компрессор КНД 0.. 16 кПа
...
#define PKizb 				RF(1091)				//Избыточное давление воздуха за КВД (0…2,5МПа)
#define Pvrpk    			RF(1093)				//Избыточное давление воздуха в разгрузочной полости КВД (0…160 кПа0
...
#define NKND1    			RF(1103)				//Частота вращения КНД- 1-й канал(6000 об/мин )
#define NKND2    			RF(1105)				//Частота вращения КНД- 2-й канал(6000 об/мин )
#define NTND1    			RF(1107)				//PЕЗЕРВ
#define NTND2    			RF(1109)				//РЕЗЕРВ
#define TmrpTnd    			RF(1111)				//Температура масла на выходе из опоры р/п ТHД (-50...250 град С)
#define TvVVT1   			RF(1113)				//Температура воздууха после ВВТ1 агрегата БВВТ-25 (-50...250 град С)
#define Pknd    			RF(1115)				//Полное давление воздуха за КНД (0…160 кПа)
#define Ptvd    			RF(1117)				//Давление газа за ТВД (0…0.6 МПа)
#define Ptnd    			RF(1119)				//Давление газа за ТНД (0…0.6 МПа)
#define deltaPst   			RF(1121)				//Перепад между атмосферным давлением и полным давлением газа за СТ 0.. 16 кПа
#define Pohltnd    			RF(1123)				//Избыточное давление воздуха охлаждения ТНД 0...1,6МПа
#define PvBVVT25    		RF(1125)				//Давление воздууха после ВВТ1 агрегата БВВТ-25 (0...1,0МПа)
#define Nstart    			RF(1127)				//Частота электростартера (0...12000 об/мин)
#define Mstart    			RF(1129)				//Частота электростартера (0...24 кгс*м)


													// RF(1130)...RF(1269) 	РЕЗЕРВ
...
#define VibegNNDTimer		RF(1567)				//Таймер времени выбега КНД
#define Ptnd_f				RF(1569)				//Давление воздуха за ТНД, 0…0.6 МПа
#define Pohltnd_f			RF(1571)				//Давление воздуха охлаждения ТНД, 0…1.6 МПа
#define PvBVVT25_f			RF(1573)				//Давление воздуха после ВВТ1 агрегата БВВт-25 0..1,0 МПа

#define N_StrFStimer_out	RF(1575)				//Таймер ВО при обнаружении стружки в ФС через 180 мин, для отображения на пульте оператора
#define N_StrKPtimer_out	RF(1577)				//Таймер ВО при обнаружении стружки в КП через 180 мин, для отображения на пульте оператора
...
#define Reg_Tst_ogr_u			RF(1929)			//Регулировка ограничения Т газа за турбиной СТ
...
#define StV_ON_tst	        BIT_TST_M(3004)			//Готовность эл. стартера
#define dPMFS_tst	        BIT_TST_M(3005)			//РЕЗЕРВ
...
#define CVStV_tst	        BIT_TST_M(3009)			//Неиспр эл стартера
#define PoutStVMax_tst	    BIT_TST_M(3010)			//РЕЗЕРВ
#define StrFS_tst	        BIT_TST_M(3014)			//Стружка в ФС
#define StrKP_tst	        BIT_TST_M(3015)			//Стружка в масле КП
#define StrSPK_tst	        BIT_TST_M(3016)			//Стружка в линии откачки масла от ШП КВД
#define StrRPT_tst	        BIT_TST_M(3017)			//Стружка в линии откачки масла от РП ТВД
#define StrST_tst	        BIT_TST_M(3018)			//Стружка в линии откачки масла от опор СТ
#define StrMStV_tst         BIT_TST_M(3019)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3020)			//Автоматы питания "Включены"
#define StrMStV_tst         BIT_TST_M(3021)			//Контроль цепи открытия СК
#define StrMStV_tst         BIT_TST_M(3022)			//Контроль цепи откртыия КПВ КВД
#define StrMStV_tst         BIT_TST_M(3023)			//Контроль цепи откртыия КПГ КПВЗ
#define StrMStV_tst         BIT_TST_M(3024)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3025)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3026)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3027)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3028)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3029)			//РЕЗЕРВ
#define ZPV1_ONpos  	    BIT_TST_M(3030)			//ЗПВ КНД 1-й гр. открыты
#define ZPV2_ONpos	    	BIT_TST_M(3031)			//ЗПВ КНД 2-й гр. открыты
#define StrMStV_tst         BIT_TST_M(3032)			//ПОС включена
#define StrRPTnd_tst        BIT_TST_M(3033)			//Стружка в линии откачки масла от РП ТНД
#define StrMStV_tst         BIT_TST_M(3034)			//Контроль цепи откртыия ЗПВ КНД 1-й гр.
#define StrMStV_tst         BIT_TST_M(3035)			//Контроль цепи откртыия ЗПВ КНД 2-й гр.
#define StrMStV_tst         BIT_TST_M(3036)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3037)			//РЕЗЕРВ
#define StrMStV_tst         BIT_TST_M(3038)			//РЕЗЕРВ
...
#define FNGG1				BIT_TST_M(3043)			//Неисправность или обрыв цепи Обороты КВД  №1                                                                        
#define FNGG1_clr 			BIT_CLR_M(3043)
#define FNGG2				BIT_TST_M(3044)			//Неисправность или обрыв цепи Обороты КВД №2                                                                        
...
#define FNKND1				BIT_TST_M(3047)			//Неисправность или обрыв цепи Обороты КНД №1     
#define FNKND1_clr 			BIT_CLR_M(3047)
#define FNKND2				BIT_TST_M(3048)			//Неисправность или обрыв цепи Обороты КНД №2     
#define FNKND2_clr 			BIT_CLR_M(3048)
...
#define PgInStVBrk			BIT_TST_M(3053)			//РЕЗЕРВ                                                                        
...
#define FPkvd				BIT_TST_M(3056)			//Обрыв цепи Полное Давление газа за КВД
#define FPVX				BIT_TST_M(3057)			//Обрыв цепи Перепад между атмосферным давлением и полным давлением воздуха на входе КНД
#define FPVXK				BIT_TST_M(3058)			//Обрыв цепи Перепад между атмосферным давлением и статическим давлением воздуха на входе КНД
#define FPGST				BIT_TST_M(3059)			//Обрыв цепи Полное Давление газа за СТ                                                                       
#define PvrpkBrk			BIT_TST_M(3060)			//Обрыв цепи Давление воздуха в разгрузочной полости КВД
...
#define Tkvd				BIT_TST_M(3072)			//Обрыв цепи Температура воздуха за компрессором КВД    
#define FTTXG1				BIT_TST_M(3073)			//Обрыв цепи Т газа за ТВД на запуске т.1                                                                        
#define FTTXG2				BIT_TST_M(3074)			//Обрыв цепи Т газа за ТВД на запуске т.2                                                                        
#define FTTXH1    			BIT_TST_M(3075)			//Обрыв цепи температура газа за ТВД на режимах 1
#define FTTXH2    			BIT_TST_M(3076)			//Обрыв цепи температура газа за ТВД на режимах 2
...
#define TmrpTndBrk			BIT_TST_M(3099)			//Обрыв цепи Т масла на выходе из опоры р/п ТНД
#define FTvBVVT25			BIT_TST_M(3100)			//Обрыв цепи Т воздуха после ВВТ1 агрегата БВВТ-25    
#define FTohltnd			BIT_TST_M(3101)			//Обрыв цепи Т воздуха охлаждения ТНД                                                                        
#define FPvBVVT25			BIT_TST_M(3102)			//Обрыв цепи Давление воздуха после ВВТ1 агрегата БВВТ-25                                                                       
#define FNstarter  			BIT_TST_M(3103)			//Обрыв цепи Частота электростартера
#define FMstarter  			BIT_TST_M(3104)			//Обрыв цепи Момент электростартера	
#define FPknd				BIT_TST_M(3105)			//Обрыв цепи Полное давление воздуха за КНД    
#define FPtvd				BIT_TST_M(3106)			//Обрыв цепи Полное давление газа за ТВД                                                                       
#define FPtnd				BIT_TST_M(3107)			//Обрыв цепи Полное давление газа за ТНД                                                                        
#define FdeltaPst    		BIT_TST_M(3108)			//Обрыв цепи перепад между атмосферным давлением и полным давлением газа за СТ
#define FPohltnd			BIT_TST_M(3109)			//Обрыв цепи Давление воздуха охлаждения ТНД
...
#define ZPV1_ONBrk			BIT_TST_M(3154)			//Обрыв цепи открытия ЗПВ КНД 1 группы
#define ZPV2_ONBrk			BIT_TST_M(3155)			//Обрыв цепи открытия ЗПВ КНД 2 группы
...
#define Nstart_tst			BIT_TST_M(3404)			//*Включить электростартер
//#define VZ_tst					BIT_TST_M(3405)			//*Проверка команды на воздушную заслонку
#define PNstart_tst			BIT_TST_M(3405)			//*Включить повышенную мощность электростартера
...
#define R_promEnd_tst		BIT_TST_M(3467)			//Проверка флага "Конец Промывки"
...
#define A_NggHigh_set		BIT_SET_M(3501)			//Уст-ка флага "Аварийно-высокая частота вращения ротора КВД"
#define A_NggHigh_clr		BIT_CLR_M(3501)			//Сброс флага "Аварийно-высокая частота вращения ротора КВД"
#define A_NggHigh_tst		BIT_TST_M(3501)			//Пр-ка флага "Аварийно-высокая частота вращения ротора КВД"
...
#define P_Ttzap_set	    	BIT_SET_M(3504)			//Уст-ка флага "Высокая Т газа  за турбиной ТВД на запуске"
#define P_Ttzap_clr		    BIT_CLR_M(3504)			//Сброс флага "Высокая Т газа  за турбиной ТВД на запуске"
#define P_Ttzap_tst		    BIT_TST_M(3504)			//Пр-ка флага "Высокая Т газа  за турбиной ТВД на запуске"

#define A_Ttzap_set	   		BIT_SET_M(3505)			//Уст-ка флага "Аварийно-высокая температура газа за ТВД на запуске"
#define A_Ttzap_clr	   	 	BIT_CLR_M(3505)			//Сброс флага "Аварийно-высокая температура газа за ТВД на запуске"
#define A_Ttzap_tst			BIT_TST_M(3505)			//Пр-ка флага "Аварийно-высокая температура газа за ТВД на запуске"

#define A_NndHigh_set	   	BIT_SET_M(3506)			//Уст-ка флага "Высокая частота вращения ротора КНД"-АО
#define A_NndHigh_clr	    BIT_CLR_M(3506)			//Сброс флага "Высокая частота вращения ротора КНД"-АО
#define A_NndHigh_tst	    BIT_TST_M(3506)			//Пр-ка флага "Высокая частота вращения ротора КНД"-АО
/*
...
#define P_PkvdHigh_set	   	BIT_SET_M(3512)			//Уст-ка флага "Повышенное давление воздуха за КВД"
#define P_PkvdHigh_clr	    BIT_CLR_M(3512)			//Сброс флага "Повышенное давление воздуха за КВД"
#define P_PkvdHigh_tst		BIT_TST_M(3512)			//Пр-ка флага "Повышенное давление воздуха за КВД"
...
#define P_VibegGGMin_set   	BIT_SET_M(3526)			//"Мало время выбега ротора КВД"
...
#define P_TtBrk_set    	BIT_SET_M(3662)				//Уст-ка флага "Отказ всех каналов Tt"
#define P_TtBrk_clr		BIT_CLR_M(3662)				//Сброс флага "Отказ всех каналов Tt"
#define P_TtBrk_tst		BIT_TST_M(3662)				//Пр-ка флага "Отказ всех каналов Tt"

#define P_TtDistort_set 	BIT_SET_M(3663)			//Уст-ка флага "Отказ всех каналов Tt, обнаруженный перекрестным контролем" 
#define P_TtDistort_clr 	BIT_CLR_M(3663)			//Сброс флага "Отказ всех каналов Tt, обнаруженный перекрестным контролем"
#define P_TtDistort_tst		BIT_TST_M(3663)			//Пр-ка флага "Отказ всех каналов Tt, обнаруженный перекрестным контролем"
...
#define P_StrSPK_set     	BIT_SET_M(3639)			//"Стружка в линии откачки масла от ШП КВД"
#define P_StrSPK_clr        BIT_CLR_M(3639)			
#define P_StrSPK_tst        BIT_TST_M(3639)		

#define N_StrRPT_set     	BIT_SET_M(3640)			//"Стружка в линии откачки масла от РП ТВД"
#define N_StrRPT_clr        BIT_CLR_M(3640)			
#define N_StrRPT_tst        BIT_TST_M(3640)	

#define P_StrST_set     	BIT_SET_M(3641)			//"Стружка в линии откачки масла от опор СТ"
#define P_StrST_clr         BIT_CLR_M(3641)			
#define P_StrST_tst         BIT_TST_M(3641)		

#define N_StrFS_set     	BIT_SET_M(3642)			//"Стружка в ФС"
#define N_StrFS_clr         BIT_CLR_M(3642)			
#define N_StrFS_tst         BIT_TST_M(3642)		

#define N_StrRPTnd_set  	BIT_SET_M(3643)			//"Стружка в масле откачки от РП ТНД"
#define N_StrRPTnd_clr      BIT_CLR_M(3643)			
#define N_StrRPTnd_tst      BIT_TST_M(3643)		
...
#define A_Pompaz_set 		BIT_SET_M(3654)			//Уст-ка флага "Аварийный останов по сигналу "Помпаж"
#define A_Pompaz_clr 		BIT_CLR_M(3654)				
#define A_Pompaz_tst 		BIT_TST_M(3654)	
...
#define A_NggLow_set			BIT_SET_M(3657)		//Уст-ка флага "Аварийно-низкая частота вращения ротора КВД"
#define A_NggLow_clr			BIT_CLR_M(3657)		//Сброс флага "Аварийно-низкая частота вращения ротора КВД"
#define A_NggLow_tst			BIT_TST_M(3657)		//Пр-ка флага "Аварийно-низкая частота вращения ротора КВД"

#define A_PompazZap_set		BIT_SET_M(3658)			//Уст-ка флага "Аварийный останов по сигналу "Помпаж на запуске"
#define A_PompazZap_clr		BIT_CLR_M(3658)			//Сброс флага "Аварийный останов по сигналу "Помпаж на запуске"
#define A_PompazZap_tst		BIT_TST_M(3658)			//Пр-ка флага "Аварийный останов по сигналу "Помпаж на запуске"
...
*/
#define P_TvVVT1High_set	BIT_SET_M(3664)			//Повышенная температура воздуха после ВВТ1
#define P_TvVVT1High_clr	BIT_CLR_M(3664)			
#define P_TvVVT1High_tst	BIT_TST_M(3664)	

#define P_TmrpTndHigh_set	BIT_SET_M(3665)			//Повышенная температура масла откачки от РП ТНД
#define P_TmrpTndHigh_clr	BIT_CLR_M(3665)			
#define P_TmrpTndHigh_tst	BIT_TST_M(3665)

#define N_TmrpTndMax_set	BIT_SET_M(3666)			//Высокая температура масла откачки от РП ТНД
#define N_TmrpTndMax_clr	BIT_CLR_M(3666)			
#define N_TmrpTndMax_tst	BIT_TST_M(3666)

#define A_TmrpTndMax_set	BIT_SET_M(3667)			//Аварийно-высокая температура масла откачки от РП ТНД
#define A_TmrpTndMax_clr	BIT_CLR_M(3667)			
#define A_TmrpTndMax_tst	BIT_TST_M(3667)

#define P_OhlTnd_set		BIT_SET_M(3668)			//Пониженное давление воздуха охлаждения ТНД
#define P_OhlTnd_clr		BIT_CLR_M(3668)			
#define P_OhlTnd_tst		BIT_TST_M(3668)

#define N_OhlTnd_set		BIT_SET_M(3669)			//Низкое давление воздуха охлаждения ТНД
#define N_OhlTnd_clr		BIT_CLR_M(3669)			
#define N_OhlTnd_tst		BIT_TST_M(3669)

#define P_LowPBBT1_set		BIT_SET_M(3670)			//Пониженное давление воздуха после ВВТ1
#define P_LowPBBT1_clr		BIT_CLR_M(3670)			
#define P_LowPBBT1_tst		BIT_TST_M(3670)

#define N_LowPBBT1_set		BIT_SET_M(3671)			//Низкое давление воздуха после ВВТ1
#define N_LowPBBT1_clr		BIT_CLR_M(3671)			
#define N_LowPBBT1_tst		BIT_TST_M(3671)

#define P_ZPV1Brk_set		BIT_SET_M(3672)			//Неисправность 1 группы ЗПВ КНД
#define P_ZPV1Brk_clr		BIT_CLR_M(3672)			
#define P_ZPV1Brk_tst		BIT_TST_M(3672)

#define P_ZPV2Brk_set		BIT_SET_M(3673)			//Неисправность 2 группы ЗПВ КНД
#define P_ZPV2Brk_clr		BIT_CLR_M(3673)			
#define P_ZPV2Brk_tst		BIT_TST_M(3673)

#define P_VibegNNDMin_set	BIT_SET_M(3674)			//Мало время выбега ротора КНД
#define P_VibegNNDMin_clr	BIT_CLR_M(3674)			
#define P_VibegNNDMin_tst	BIT_TST_M(3674)

#define P_Readystarter_set	BIT_SET_M(3675)			//Готовность электростартера
#define P_Readystarter_clr	BIT_CLR_M(3675)			
#define P_Readystarter_tst	BIT_TST_M(3675)

#define P_Neispstarter_set	BIT_SET_M(3676)			//Неисправность электростартера
#define P_Neispstarter_clr	BIT_CLR_M(3676)			
#define P_Neispstarter_tst	BIT_TST_M(3676)

#define P_LowTimeknd_set	BIT_SET_M(3677)			//Отказ канала измерения температуры масла на выходе из опоры РП ТНД, обнаруженный допусковым контролем 
#define P_LowTimeknd_clr	BIT_CLR_M(3677)			
#define P_LowTimeknd_tst	BIT_TST_M(3677)

#define P_LowTimeknd_set	BIT_SET_M(3678)			//Отказ канала измерения температуры воздуха после ВВТ1 агрегата БВВТ-25 , обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3678)			
#define P_LowTimeknd_tst	BIT_TST_M(3678)

#define P_LowTimeknd_set	BIT_SET_M(3679)			//Отказ канала измерения частоты электростартера, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3679)			
#define P_LowTimeknd_tst	BIT_TST_M(3679)

#define P_LowTimeknd_set	BIT_SET_M(3680)			//Отказ канала измерения момента электростартера, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3680)			
#define P_LowTimeknd_tst	BIT_TST_M(3680)

#define P_LowTimeknd_set	BIT_SET_M(3681)			//Отказ канала измерения частоты вращения ротора КНД 1 канал, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3681)			
#define P_LowTimeknd_tst	BIT_TST_M(3681)

#define P_LowTimeknd_set	BIT_SET_M(3682)			//Отказ канала измерения частоты вращения ротора КНД 2 канал, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3682)			
#define P_LowTimeknd_tst	BIT_TST_M(3682)

#define P_NeispZPV2_set		BIT_SET_M(3683)			//Отказ всех каналов измерения частоты вращения ротора КНД
#define P_NeispZPV2_clr		BIT_CLR_M(3683)			
#define P_NeispZPV2_tst		BIT_TST_M(3683)

#define P_LowTimeknd_set	BIT_SET_M(3684)			//Отказ всех каналов измерения частоты вращения ротора КНД, обнаруженный перекрестным контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3674)			
#define P_LowTimeknd_tst	BIT_TST_M(3684)

#define P_LowTimeknd_set	BIT_SET_M(3685)			//Отказ канала измерения Давления воздуха за компрессором КНД, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3685)			
#define P_LowTimeknd_tst	BIT_TST_M(3685)

#define P_LowTimeknd_set	BIT_SET_M(3686)			//Отказ канала измерения Давления газа за ТВД, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3686)			
#define P_LowTimeknd_tst	BIT_TST_M(3686)

#define P_LowTimeknd_set	BIT_SET_M(3687)			//Отказ канала измерения Ддавления газа за ТНД, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3687)			
#define P_LowTimeknd_tst	BIT_TST_M(3687)

#define P_LowTimeknd_set	BIT_SET_M(3688)			//Отказ канала измерения перепада между атмосферным давлением и полным давлением газа за СТ, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3688)			
#define P_LowTimeknd_tst	BIT_TST_M(3688)

#define P_LowTimeknd_set	BIT_SET_M(3689)			//Отказ канала измерения Давления воздуха охлаждения ТНД, обнаруженный допусковым контролем
#define P_LowTimeknd_clr	BIT_CLR_M(3689)			
#define P_LowTimeknd_tst	BIT_TST_M(3689)

#define P_NeispZPV2_set		BIT_SET_M(3690)			//Отказ канала измерения Давления воздуха после ВВТ1 агрегата БВВТ, обнаруженный допусковым контролем
#define P_NeispZPV2_clr		BIT_CLR_M(3690)			
#define P_NeispZPV2_tst		BIT_TST_M(3690)

#define P_LowTimeknd_set	BIT_SET_M(3691)			//Неисправность ПОС
#define P_LowTimeknd_clr	BIT_CLR_M(3691)			
#define P_LowTimeknd_tst	BIT_TST_M(3691)

#define P_LowTimeknd_set	BIT_SET_M(3692)			//Нет включения компрессора ТНД
#define P_LowTimeknd_clr	BIT_CLR_M(3692)			
#define P_LowTimeknd_tst	BIT_TST_M(3692)

#define A_ZPV1IMBrk_set		BIT_SET_M(3693)			//Отказ электр. части канала управления ЗПВ КНД 1 группы
#define A_ZPV1IMBrk_clr		BIT_CLR_M(3693)			
#define A_ZPV1IMBrk_tst		BIT_TST_M(3693)

#define A_ZPV2IMBrk_set		BIT_SET_M(3694)			//Отказ электр. части канала управления. ЗПВ КНД 2 группы
#define A_ZPV2IMBrk_clr		BIT_CLR_M(3694)			
#define A_ZPV2IMBrk_tst		BIT_TST_M(3694)
...
static float	Tvh1, Tvh2, Tvh3, Tvh4;//Переменные для фильтрации Tvh
...
static float	Tt1, Tt2, Tt3, Tt4;					//Переменные для фильтрации Tt
...
static float	ZPV1BrkTimer;
static float	ZPV2BrkTimer;
...
static float	TmvhDmax2Timer;
static float	PogasTimer;
static float	TtzapTimer;
static float	ATtzapTimer;
...
static float 	Reg_Tst_ogr;					//Регулировка ограничения Т газа за турбиной СТ
...
static float	Reg_Nnd_Max;					//Регулировка максимальной частоты вращения ротора НД
static float	P_StrSPKtimer;
static float	N_StrRPTtimer;
static float	P_StrSTtimer;
static float	N_StrRPTndtimer;
static float	N_StrFStimer;
static float	P_StrKPtimer;
static float	N_StrKPtimer;
static float	P_StrFStimer;
...
static float	Pk_maxTimer;
static float	Nnd_ogrTimer;
static float	TtgpdLowTimer;
static float	TtgpdHighTimer;
static float	P_TvVVT1Ttimer;
static float	PohltndTimer;
static float	PvBVVT25Timer;
static float	A_STBrkTimer;

//Переменные для алгоритма помпажа  
static float	PompazTimer;
static float P_I[8],Pmax,Pmin;
int ict, PmaxI, PminI;
...
if ((Reg_Tst_ogr_u >= -600.0)&&(Reg_Tst_ogr_u <= 300.0)) Reg_Tst_ogr = Reg_Tst_ogr_u;
...
if ((Reg_Nnd_Max_u >= -500.0)&&(Reg_Nnd_Max_u <= 500.0)) Reg_Nnd_Max = Reg_Nnd_Max_u; 
}
...
//Обработка Тт по двум каналам
	if (externDeblock) {P_TtBrk_clr;P_TtDistort_clr;}
	
	if (FTTXH1||P_Tt1BrkDop_tst)
		if (FTTXH2||P_Tt2BrkDop_tst) P_TtBrk_set;
			else Tt1=TTXH2+273;
	  else if (FTTXH2||P_Tt2BrkDop_tst) Tt1=TTXH1+273;
			   else if (abs(TTXH2-TTXH1)>maxDeltaTtStart) P_TtDistort_set;
						else  { if (TTXH2>TTXH1) Tt1=TTXH2+273;
								   else Tt1=TTXH1+273;
							   }

	if ((condition==6)||(condition==1))
		if (TTXH2>TTXH1) Tt1=TTXH2+273;
			else Tt1=TTXH1+273;

	if (((!P_TtBrk_tst)&&(!P_TtDistort_tst))||(condition==6)||(condition==1))
	{ Tt=(Tt1 + Tt2 + Tt3 + Tt4)/4;
	  Tt4=Tt3;
	  Tt3=Tt2;
	  Tt2=Tt1;
	}
...
//Проверка каналов измерения Тcт и вычисление Тcт
	...
	if ((condition==6)||(condition==1)) {P_Tst1BrkOtkl_clr;P_Tst2BrkOtkl_clr;P_Tst3BrkOtkl_clr;}
...
//Предупредительная сигнализация по ВТЗ
	 ...
	 if (TtzapTimer > 1.0) TtzapTimer = 1.0;
	 if (TtzapTimer >= 1.0) P_Ttzap_set;						//"Высокая температура газа  за турбиной ТВД на запуске"
		else if (externDeblock) P_Ttzap_clr;
//Прекращение запука по ВТЗ		
	 if ((Tt>Ttzap_ogr_ust+50)&&(nvd<(Ngg_MG_ust-deltaNvdLow))) ATtzapTimer +=cycle; else ATtzapTimer = 0.0;
	 if (ATtzapTimer > 0.5) ATtzapTimer = 0.5;
	 if (ATtzapTimer >= 0.5) A_Ttzap_set;						//"Аварийно-Высокая температура газа  за турбиной ТВД на запуске"
...
//АО по провалу оборотов ниже минимального режима, уставка минимального режима
	...
	if (Ngg_MGTimer >= 0.1) A_NggLow_set;							//"Аварийно-низкая частота вращения ротора КВД"
...
//Коэффициент коррекции nvd по Твх, уставка максимального режима, АО по максимальным оборотам ГГ
	...
	if (Ngg_maxTimer >= 0.1) A_NggHigh_set;							//"Аварийно-высокая частота вращения ротора КВД"	
...
//Уставка ограничения максимальных оборотов НД
	Nnd_max_ust=NndMax+Reg_Nnd_Max;
	if (nnd>=NndAlarm) Nnd_ogrTimer +=cycle; else Nnd_ogrTimer=0;
	if (Nnd_ogrTimer > 0.1) Nnd_ogrTimer=0.1;
	if (Nnd_ogrTimer >= 0.1) A_NndHigh_set;							// "Высокая частота вращения ротора КНД"
		else if (externDeblock) A_NndHigh_clr;
//Уставка ограничения величины давления Рк
	if (maxReg) Pk_ogr_ust=(Pk_ogr_max+Reg_Pk_ogr);							
		else Pk_ogr_ust=(Pk_ogr_nom+Reg_Pk_ogr);
	if (Pkvd>=Pk_ogr_Alarm) Pk_maxTimer +=cycle; else Pk_maxTimer=0;
	if (Pk_maxTimer > 0.25) Pk_maxTimer=0.25;
	if (Pk_maxTimer >= 0.25) P_PkvdHigh_set;							// "Повышенное давление воздуха за КВД"
		else P_PkvdHigh_clr;
...
//Уставка ограничения температуры газа за СТ на режимах, АО по максимальной Тст
	if (maxReg) Tst_ogr_ust=(TstMax + Reg_Tst_ogr)*harakt(CTstTinMax,Tvh);
		else Tst_ogr_ust=(TstMax + Reg_Tst_ogr)*harakt(CTstTin,Tvh);
	if ((nvd>(Ngg_MG_ust-deltaNvdLow))||check)
		{if (Tst>(Tst_ogr_ust+DeltaTst3)) A_TstHigh_set;						//"Аварийно-высокая температура газа  за СТ"
			else {if (externDeblock) A_TstHigh_clr;
				  if (Tst>(Tst_ogr_ust+DeltaTst2)) N_TstHigh_set;				//"Высокая температура газа  за СТ
					else {if (externDeblock) N_TstHigh_clr;
						  if (Tst>(Tst_ogr_ust+DeltaTst1)) P_TstHigh_set;		//"Повышенная температура газа  за СТ"
							else P_TstHigh_clr;}
					}}
		else if (externDeblock) {A_TstHigh_clr; N_TstHigh_clr; P_TstHigh_clr;}
...
//Контроль температуры масла на входе в двигатель
	...
	if (TmvhDminTimer > 20.0) TmvhDminTimer = 20.0;	 
	if (TmvhDminTimer == 20.0) P_TmvhDmin_set;       	//"Пониженная температура масла на входе в двигатель"
		else P_TmvhDmin_clr;
	...
	if (TmvhD>=TmvhDHigh2) TmvhDmax2Timer += cycle; else TmvhDmax2Timer = 0.0;
	if (TmvhDmax2Timer > 60.0) TmvhDmax2Timer = 60.0;	//"Для нормального останова Повышенная температура масла на входе в двигатель"	 
...
//Контроль давления газа перед СК
	if ((Ptg<=PtgMinLim)&&((condition>2)&&(!HP_tst)&&(!endHP_tst)&&(!R_promGo_tst)&&(!R_promEnd_tst)&&(condition!=6)||check)) {A_PtgMin_set;P_PtgMin_clr;P_PtgLow_clr;}	//"Аварийно-низкое давление топливного газа перед СК"
		else {if (externDeblock) A_PtgMin_clr;
			  if ((Ptg<=PtgMin)&&((condition>2)&&(!HP_tst)&&(!endHP_tst)&&(!R_promGo_tst)&&(!R_promEnd_tst)&&(condition!=6)||check)) {P_PtgMin_set;P_PtgLow_clr;}		//"Низкое давление топливного газа перед СК"
				else { P_PtgMin_clr;
	...
	if (PtgHighTimer > 5.0) PtgHighTimer = 5.0;
	if ((PtgHighTimer == 5.0)&&((condition>4)||check)) {N_PtgHigh_set;P_PtgHigh_clr;}
...
//Контроль температуры топливного газа перед ДГ
	if (Tpdtg<=TtgpdLow) TtgpdLowTimer += cycle; else TtgpdLowTimer = 0.0;
	if (TtgpdLowTimer > 1.0) TtgpdLowTimer = 1.0;	 
	if (TtgpdLowTimer == 1.0) P_TtgMin_set;				//Пониженная температура топливного газа перед ДГ
		else P_TtgMin_clr;

	if (Tpdtg>=TtgpdHigh) TtgpdHighTimer += cycle; else TtgpdHighTimer = 0.0;
	if (TtgpdHighTimer > 1.0) TtgpdHighTimer = 1.0;	 
	if (TtgpdHighTimer == 1.0) P_TtgMax_set;			//Повышенная температура топливного газа перед ДГ
		else P_TtgMax_clr;
...
PvrpST=PizbST + 0.1;	
	if ((nvd>9700)||check)
		{if ((PvrpST/Pkvd)<0.22) P_osSTHigh_set; 							//Повышенная осевая сила ротора СТ
			else P_osSTHigh_clr;}
	  else P_osSTHigh_clr;
...
//Контроль температуры масла на выходе из опоры р/п ТНД				
	if ((nvd>=TmrpTndContr)||check)
		{if(TmrpTnd>=TmrpTndAlarm) {A_TmrpTndMax_set;P_TmrpTndHigh_clr;}			//"Аварийно-высокая температура масла на выходе из опоры р/п ТНД"     !!!
		   else {if (externDeblock) A_TmrpTndMax_clr;
			     if (TmrpTnd>=TmrpTndMax) {N_TmrpTndMax_set;P_TmrpTndHigh_clr;}		//"Высокая температура масла на выходе из опоры р/п ТНД"
				    else {if (externDeblock) N_TmrpTndMax_clr;
					      if (TmrpTnd>=TmrpTndHigh) P_TmrpTndHigh_set;				//"Повышенная температура масла на выходе из опоры р/п ТНД"
						    else P_TmrpTndHigh_clr;}
				}
		}
		   else {P_TmrpTndHigh_clr;
				if (externDeblock) {A_TmrpTndMax_clr;N_TmrpTndMax_clr;}
				}				
...
// Готовность электростартера
if (StV_ON_tst) P_Readystarter_set;								
		else P_Readystarter_clr;
// Неисправность электростартера
if (CVStV_tst) P_Neispstarter_set;								
		else P_Neispstarter_clr;

//Контроль наличия стружки ИЗМЕНЕНО В СООТВЕТСТВИИ С РЕШЕНИЕМ № 2009-313
	if (StrKP_tst) P_StrKPtimer += cycle;else P_StrKPtimer = 0.0;//"Стружка в масле КП"
	if (P_StrKPtimer >= 1.0) P_StrKPtimer = 1.0;
	if (P_StrKPtimer == 1.0) N_StrKP_set;						
	if (N_StrKP_tst) N_StrKPtimer += cycle;	
	if (N_StrKPtimer >= 10800.0) N_StrKPtimer = 10800.0;
	N_StrKPtimer_out = N_StrKPtimer;
	if ((externDeblock) && ((N_StrKPtimer == 10800.0) && N_StrKP_tst)) {N_StrKP_clr; N_StrKPtimer = 0.0;}
	if ((externDeblock) && ((condition == 1)||(condition == 6))) {N_StrKP_clr; N_StrKPtimer = 0.0;}

	if (StrSPK_tst) P_StrSPKtimer += cycle;else P_StrSPKtimer  = 0.0;//"Стружка в линии откачки масла от ШП КВД"  
	if (P_StrSPKtimer >= 5.0) P_StrSPKtimer  = 5.0;
	if (P_StrSPKtimer == 5.0) P_StrSPK_set;						
		else P_StrSPK_clr;	
	
	if (StrRPT_tst) N_StrRPTtimer += cycle;else N_StrRPTtimer  = 0.0;//"Стружка в линии откачки масла от РП ТВД"
	if (N_StrRPTtimer >= 5.0) N_StrRPTtimer  = 5.0;
	if (N_StrRPTtimer == 5.0) N_StrRPT_set;						
		else if (externDeblock) N_StrRPT_clr;	
	
	if (StrST_tst) P_StrSTtimer += cycle;else P_StrSTtimer  = 0.0;//"Стружка в линии откачки масла от опор СТ"
	if (P_StrSTtimer >= 5.0) P_StrSTtimer  = 5.0;
	if (P_StrSTtimer == 5.0) P_StrST_set;						
		else P_StrST_clr;	

	if (StrFS_tst) P_StrFStimer += cycle;else P_StrFStimer  = 0.0;//"Стружка в ФС"
	if (P_StrFStimer >= 1.0) P_StrFStimer  = 1.0;
	if (P_StrFStimer == 1.0) N_StrFS_set;						
	if (N_StrFS_tst) N_StrFStimer += cycle;	
	if (N_StrFStimer >= 10800.0) N_StrFStimer = 10800.0;
	N_StrFStimer_out = N_StrFStimer;
	if ((externDeblock) && ((N_StrFStimer == 10800.0) && N_StrFS_tst)) {N_StrFS_clr; N_StrFStimer = 0.0;}		
	if ((externDeblock) && ((condition == 1)||(condition == 6))) {N_StrFS_clr; N_StrFStimer = 0.0;}	

	if (StrRPTnd_tst) N_StrRPTndtimer += cycle;else N_StrRPTndtimer  = 0.0;//"Стружка в линии откачки масла от опор РП ТНД"
	if (N_StrRPTndtimer >= 5.0) N_StrRPTndtimer  = 5.0;
	if (N_StrRPTndtimer == 5.0) N_StrRPTnd_set;						
		else if (externDeblock) N_StrRPTnd_clr;	
//Контроль АО от БЗД
...
//Контроль давления воздуха охлаждения ТНД		
Pohltnd_f = Pohltnd + 0.1;	
	if (((Pkvd>Pkvdohl)&&(!up)&&(!down))||check) PohltndTimer += cycle; else PohltndTimer = 0;
	if 	(PohltndTimer > 10000.0) PohltndTimer = 10000.0;	
	if ((Pohltnd_f <= 0.186*Pkvd)&&(PohltndTimer>PohltndTime)) {N_OhlTnd_set;P_OhlTnd_clr;}		// Низкое давление воздуха охлаждения ТНД
			else {if (externDeblock) N_OhlTnd_clr;}
	
	if ((Pohltnd_f <= 0.19*Pkvd)&&(PohltndTimer>Pohltnd1Time)) P_OhlTnd_set; else P_OhlTnd_clr;	// Пониженное давление воздуха охлаждения ТНД

//Контроль давления воздуха после ВВТ1
PvBVVT25_f = PvBVVT25 + 0.1;	
	if (((Pkvd>Pkvdohl)&&(!up)&&(!down))||check) PvBVVT25Timer += cycle; else PvBVVT25Timer = 0;
	if 	(PvBVVT25Timer > 10000.0) PvBVVT25Timer = 10000.0;	
	if ((PvBVVT25_f <= (0.07*Pkvd + 0.65))&&(PvBVVT25Timer>PvBVVT25Time)) {N_LowPBBT1_set;P_LowPBBT1_clr;}		// Низкое давление воздуха после ВВТ1
			else {if (externDeblock) N_LowPBBT1_clr;}
	
	if ((PvBVVT25_f <= (0.08*Pkvd + 0.65))&&(PvBVVT25Timer>PvBVVT251Time)) P_LowPBBT1_set; else P_LowPBBT1_clr;	// Пониженное давление воздуха после ВВТ1
	
//Контроль температуры воздуха после ВВТ1
	if ((nvd>TVVT1Contr)||check)
		{if (TvVVT1>TVVT1High) P_TvVVT1Ttimer += cycle;else P_TvVVT1Ttimer  = 0.0;
		if (P_TvVVT1Ttimer >= 1.0) P_TvVVT1Ttimer  = 1.0;
		if (P_TvVVT1Ttimer == 1.0) P_TvVVT1High_set;
			else P_TvVVT1High_clr;}
	  else P_TvVVT1High_clr;
...
if (A_ObRaskrST_timer > 1.0) A_ObRaskrST_timer = 1.0;
	if ((nvd<=1600)&&(nst>=300)&&(A_ObRaskrST_timer == 1.0)&&(!HP_tst)&&(!endHP_tst)&&(!R_promGo_tst)&&(!R_promEnd_tst)) A_ObRaskrST_set;		//"Обратная раскрутка ротора СТ"
		else if (externDeblock) A_ObRaskrST_clr;

//Контроль времени выбега КВД
if ((!(Nstart_tst || PNstart_tst)) && (nvd < 1100) && (nvd > 100))	VibegGGTimer+=cycle;								//"Мало время выбега ротора КВД"
if ((!(Nstart_tst || PNstart_tst)) && (nvd < 100) && (nvd > 80) && (((VibegGGTimer < 100) && (!endHP_tst)&&(!R_promEnd_tst)) || ((VibegGGTimer < 80) && (endHP_tst)&&(!R_promEnd_tst)))) P_VibegGGMin_set;
		 
//Контроль времени выбега СТ
if ((!(Nstart_tst || PNstart_tst)) && (nst < 1100) && (nst > 100))	VibegSTTimer+=cycle;								//"Мало время выбега ротора СТ"
if ((!(Nstart_tst || PNstart_tst)) && (!endHP_tst) && (!R_promEnd_tst) && (nst < 100) && (nst > 80) && (VibegSTTimer < 50)) P_VibegSTMin_set;

//Контроль времени выбега КНД
if ((!(Nstart_tst || PNstart_tst)) && (nnd < 1100) && (nnd > 100))	VibegNNDTimer+=cycle;								//"Мало время выбега ротора КВД"
if ((!(Nstart_tst || PNstart_tst)) && (!endHP_tst) && (!R_promEnd_tst) && (nnd < 100) && (nnd > 80) && (VibegNNDTimer < 150)) P_VibegNNDMin_set;
		 
if (externDeblock) {P_VibegGGMin_clr;P_VibegSTMin_clr;P_VibegNNDMin_clr;}													
if (Nstart_tst || PNstart_tst) {VibegGGTimer = 0.0;VibegSTTimer = 0.0;VibegNNDTimer = 0.0;P_VibegGGMin_clr;P_VibegSTMin_clr;P_VibegNNDMin_clr;}

//"Неисправность СТ"
if ((nvd>NggSTBrk) || check)
	{A_STBrkTimer = A_STBrkTimer + cycle;
	if (A_STBrkTimer > 0.5) A_STBrkTimer = 0.5;
	if (A_STBrkTimer == 0.5)
	{if (((Tst - Tstold) >= 50.0) && ((nst - nstold) <= - 75.0)) A_STbrk_set;
		else  if (externDeblock) A_STbrk_clr;
	Tstold = Tst; nstold = nst;A_STBrkTimer = 0.0;}	
	}
 else {Tstold = Tst; nstold = nst;A_STBrkTimer = 0.0;}
 if (externDeblock) A_STbrk_clr;
...
//Проверка клапанов
	...
	if ((ZPV1_ON_tst != ZPV1_ONpos)&&(nvd>nGGcontrKPV)) ZPV1BrkTimer +=cycle;else ZPV1BrkTimer=0.0;
	if (ZPV1BrkTimer >= ZPV1BrkTime) ZPV1BrkTimer = ZPV1BrkTime;
	if (ZPV1BrkTimer >= ZPV1BrkTime) P_ZPV1Brk_set;else P_ZPV1Brk_clr;
	
	if ((ZPV2_ON_tst != ZPV2_ONpos)&&(nvd>nGGcontrKPV)) ZPV2BrkTimer +=cycle;else ZPV2BrkTimer=0.0;
	if (ZPV2BrkTimer >= ZPV2BrkTime) ZPV2BrkTimer = ZPV2BrkTime;
	if (ZPV2BrkTimer >= ZPV2BrkTime) P_ZPV2Brk_set;else P_ZPV2Brk_clr;
	
	if (ZPV1_ONBrk&&(nvdNorm<(Ngg_MG_ust-30))) A_ZPV1IMBrk_set;else if (externDeblock) A_ZPV1IMBrk_clr;
	
	if (ZPV2_ONBrk&&(nvdNorm<(Ngg_MG_ust-30))) A_ZPV2IMBrk_set;else if (externDeblock) A_ZPV2IMBrk_clr;
...
//Проверка ВНА
	...
	if (((nvd>nGGcontrVNA)&&setSK_ON_tst)||check)
...
/////////////////////////////////////////////////////////////////		
//Алгоритм формирования признака помпажа
//for (ict = 0; ict < 8; ict++) P_I[ict] = P_I[ict+1];
for (ict = 0; ict < 7; ict = ict + 1) P_I[ict] = P_I[ict+1];
P_I[7] = PKizb;
...
//for (ict = 1; ict < 8; ict =+ 1) {
for (ict = 1; ict < 8; ict = ict + 1) {
if (Pmax  < P_I[ict]) {Pmax = P_I[ict]; PmaxI = ict;}
if (Pmin  > P_I[ict]) {Pmin = P_I[ict]; PminI = ict;}
}

if ((PmaxI != PminI) &&(nvd > Ngg_MG_ust)){
if   ((((Pmax - Pmin)/Pmax) > 0.4) && (((Pmax - Pmin)/(Pmax*abs(PmaxI - PminI)*cycle))>7)) A_Pompaz_set;}
...
if (PompazTimer >= 0.1) A_PompazZap_set; 
	else if (externDeblock) A_PompazZap_clr;	
...
//Сборка требований НО
	if (/*N_TtHigh_tst||*/N_PtgHigh_tst||N_PmvhDmin_tst||N_PsufMax_tst||N_deltaPvxMax_tst||N_TmspKvdMax_tst||N_TmrpTvdMax_tst||N_TstHigh_tst||
		N_VibPpMax_tst||N_VibZpMax_tst||N_VibDMax_tst||N_TmrpTndMax_tst||N_OhlTnd_tst||N_LowPBBT1_tst||(P_TmvhDmax_tst && (TmvhDmax2Timer == 60.0))||
		(N_StrKP_tst && (N_StrKPtimer == 10800.0))||N_TmopStMax_tst||N_StrRPT_tst||N_StrRPTnd_tst||N_TtBrk_tst||(N_StrFS_tst && (N_StrFStimer == 10800.0))||N_TstBrk_tst||externNO) N_common_set;
		else if (externDeblock) N_common_clr;

//!!!!!	if (externAO) P_NstHigh_set else if (externDeblock) P_NstHigh_clr;!

//Сборка аварий!
	if (A_NvdBrk_tst||A_NvdDistort_tst||A_Ttzap_tst||A_Pompaz_tst||A_PompazZap_tst||A_NstBrk_tst||A_NstDistort_tst||A_TtTtzapBrk_tst||
		A_NggLow_tst||A_NggHigh_tst||/*A_TtHigh_tst||*/A_Pogas_tst||A_TstHigh_tst||A_NstHigh_tst||A_ObRaskrST_tst||A_NndHigh_tst||
		...
		A_PmvhDmin_tst||A_TmspKvdMax_tst||A_TmrpTvdMax_tst||A_TmopStMax_tst||A_TmrpTndMax_tst||
		...
		(A_ZPV1IMBrk_tst&&(condition>1)&&(!HP_tst)&&(condition!=6))||
		(A_ZPV2IMBrk_tst&&(condition>1)&&(!HP_tst)&&(condition!=6))||
		(P_Neispstarter_tst&&(nvd<8500.0))
		) A_common_set;
		else if (externDeblock) A_common_clr;

//Сборка готовностей
	if ((nvd<NggGotovHPZP)&&(!P_PtgHigh_tst)&&(!P_PtgLow_tst)&&(!P_TmvhDmin_tst)&&(!CVStV_tst)&&(!A_KPVIMBrk_tst)&&(!A_KPGIMBrk_tst)&&(!A_ZPV1IMBrk_tst)&&(!A_ZPV2IMBrk_tst)&&/*(!A_StrMStV_tst)&&(PgInStV>PgInStVLow)&&(PgInStV<PgInStVHigh)&&*/
		/*(!A_CVStVMax_tst)&&(!A_StrMStV_tst)&&(!A_PoutStVMax_tst)&&*/(!A_common_tst)&&(!N_common_tst)&&(!P_UTtgKS_tst)&&(StV_ON_tst)&&(!startProm)&&(!startHP)&&(!N_StrKP_tst)&&(!N_StrFS_tst)) GotovZap_set;
		else GotovZap_clr;

// Готовность к холодной прокрутке		
	if (startHP && ((nvd<NggGotovHPZP)&&(!CVStV_tst)&&/*A_StrMStV_tst)&&(PgInStV>PgInStVLow)&&(PgInStV<PgInStVHigh)&&*/
		/*(!A_PoutStVMax_tst)&&*/(!A_common_tst)&&(!N_common_tst)&&(!P_UTtgKS_tst)&&(StV_ON_tst)&&(!startProm)&&(!N_StrKP_tst)&&(!N_StrFS_tst))) GotovHP_set;
		else GotovHP_clr;
	
// Готовность к промывке
	if (startProm && ((nvd<NggGotovHPZP)&&(!CVStV_tst)&&/*(!A_StrMStV_tst)&&*PgInStV>PgInStVLow)&&(PgInStV<PgInStVHigh)&&*/
		/*(!A_PoutStVMax_tst)&&*/(!A_common_tst)&&(!N_common_tst)&&(!P_UTtgKS_tst)&&(StV_ON_tst)&&(!startHP)&&(!N_StrKP_tst)&&(!N_StrFS_tst))) GotovProm_set;
		else GotovProm_clr;
