...
//Контроль наличия стружки ИЗМЕНЕНО В СООТВЕТСТВИИ С РЕШЕНИЕМ № 2009-313
	...
	if (P_StrKPtimer >= 5.0) P_StrKPtimer = 5.0;
	if (P_StrKPtimer == 5.0) N_StrKP_set;						
	if (N_StrKP_tst) N_StrKPtimer += cycle;	
	if (N_StrKPtimer >= 300.0) N_StrKPtimer = 300.0;
	N_StrKPtimer_out = N_StrKPtimer;
	if ((externDeblock) && ((N_StrKPtimer == 300.0) && N_StrKP_tst)) {N_StrKP_clr; N_StrKPtimer = 0.0;}
	...
	if (P_StrFStimer >= 5.0) P_StrFStimer  = 5.0;
	if (P_StrFStimer == 5.0) N_StrFS_set;						
	if (N_StrFS_tst) N_StrFStimer += cycle;	
	if (N_StrFStimer >= 300.0) N_StrFStimer = 300.0;
	N_StrFStimer_out = N_StrFStimer;
	if ((externDeblock) && ((N_StrFStimer == 300.0) && N_StrFS_tst)) {N_StrFS_clr; N_StrFStimer = 0.0;}		
...
//Сборка требований НО
	if (N_TtHigh_tst||N_PtgHigh_tst||N_PmvhDmin_tst||N_PsufMax_tst||N_deltaPvxMax_tst||N_TmspKvdMax_tst||N_TmrpTvdMax_tst||N_TstHigh_tst||
		...
		(N_StrKP_tst && (N_StrKPtimer == 300.0))||N_TmopStMax_tst||N_StrRPT_tst||N_TtBrk_tst||(N_StrFS_tst && (N_StrFStimer == 300.0))||N_TstBrk_tst||externNO) N_common_set;
