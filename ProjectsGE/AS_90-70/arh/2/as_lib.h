...
void REG_PID_SEL(/*Входы*/PV, SP, PV_Scale, ON, APR_MA, M_SP, Kp, Ki, loLimOut,
...
float *Ki;
float *CV; 
float *Act; //Номер активного контура Act[3];
DSEL *as_MStruct;
{
...
SP3= Mashtab (SP[2], PV_Scale[0], PV_Scale[1], 0.0, 100.0);
...
as_MStruct->pid_struct[1].DB = 1.0;
...
int a= SIG_SEL(as_MStruct, test_reg);
...
void  POC (PS, ON,OFF, IN, TS, IN_F, Usl_OF, SpeedUP_dP, SpeedUP_P_Ku,CVRegSpeed,dP_max, MA_AS, POC_P, Time, end_POC, CV_P,POC_P_On1)
short PS;
short ON;
short OFF;
float IN;
