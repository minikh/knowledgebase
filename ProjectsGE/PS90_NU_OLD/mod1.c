...
int GefMain(void) { //начало прграммы
...
if (init_tst)  {// инициализация
    ...
    if (Nst<10)  Nst =10;
	   ...
	   NvdToNndfunc	   =harakt(NvdToNnd,    Nvd0);	  
...
//      else YdusMain = YdusMain -integStep*DUS_RATE;
    ...
    YdusMainToGtfunc =harakt(YdusMainToGt, DG_POZ);
    Gt=YdusMainToGtfunc*Ptg/1.9; // сюда надо добавить коррекцию по давлению ТГ      
	...
	if (Gt>50) Nst=Nst+cycle*30000*((Nvd0ToPowfunc+Nvd0ToA1func*(Gt0t-Gt0n)/Gt0n)/(kNormT*kNormP)-POW_COEFF*Nst*Nst)/((J_ST+J_N)*M_PI*Nst);
...
//RF(1141) = Tt;//для отладки
	...
	PK_I = (double)Pkvd/10; // + RW(1160);   							//Изб. давление воздуха за компрессором 0...2.5 Мпа
