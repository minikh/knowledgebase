...
T_OUT= IN[4];	//Температура газа на выходе нагнетателя
DP= IN[5];		//Перепад газа на конфузоре нагнетателя
PL_G= IN[6];	//Плотность газа
B= IN[7];		//Барометрическое давление
SP_M= IN[8];
TSCAN= IN[9];
...
MASS(Q_OK,NST,Pin,Pout,T_IN,T_OUT,dP,pl_g,B, //Входы
     Nnom,Q_min,KKN,	//Константы
	 &Rc,&Q_pr,&Q,&Ky,&Ni,&G);	//Выходы
