...
//изменение 2012.09.27
//изменение 2012.10.09
...
float hiLim,hiLim_max,dLim,loLim,ver,PL_G,P_IN,Pin,P_OUT,Pout,DP,T_IN;
...
On_RUN= (!Prod_On && !iAO /*&& !SR && !MA_AS*/ && (MD_STOP || MD_RUN || MD_ZAGR) && APK_ON_const);
if (On_RUN) APR_MA= 1;
...
LIM_SPEED_2_1 (LineSP, LS_SP_DN, -1 * LS_SP_DN,&SP_Ky);
...
if (((Pout>=(P_out_Lim_SP-Pout_DB)) || (Rc>=(Rc_Lim_SP-Rc_DB))) && (!MD_RAZGR)) 
	{
	APR_MA = 0;
	MA_AS = 0;
	}
//для устранения зашкаливания CV_RT вводим фиксатор верхнего уровня для CV_Reg
dLim= loLim+10.0; 
hiLim_max= 100.0 + Step_First_var - CV_RT;
if (hiLim_max < dLim) hiLim_max= dLim;

//void REG_PID_SEL(*PV, *SP, *Contur_ON, APR_MA, M_SP, *Kp, *Ki, loLimOut,*CV,*Act,*as_MStruct)	
///*************************************************************************************************************************************
REG_PID_SEL(PV,SP,PV_Scale,&Contur_ON,APR_MA,SP_M_Current,AS_Kp,AS_Ki,AS_DB,loLim,hiLim_max,&CV_Reg,&Act,&as_MStruct);
